"""
2D Linear Elasticity Beam Analysis using Physics-Informed Neural Networks (PINNs).

This module implements a PINN-based solver for 2D linear elasticity problems,
specifically modeling a cantilever beam under shear loading. It solves the
plane stress momentum equations with mixed Dirichlet and Neumann boundary
conditions, compares predictions against analytical solutions, and exports
results in VTK format for visualization.

Key components:
- Geometry: 2D rectangular beam mesh generated via Gmsh
- PDE: 2D plane stress momentum equations
- BCs: Fixed left end, free-slip top/bottom, shear load on right end
- Network: Feed-forward neural network with output scaling
- Validation: Error analysis against analytical elasticity solution
