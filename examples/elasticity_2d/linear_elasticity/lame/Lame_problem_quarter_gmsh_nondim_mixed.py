...
characteristic_disp = (
    1 / youngs_modulus_analy * pressure_inlet
...
def compareModelPredictionAndAnalyticalSolution(model):
    ...
    u_rad = (
        ...
        * pressure_inlet
        * r
        / (youngs_modulus_analy * (radius_outer**2 - radius_inner**2))
        * (1 - nu_analy + (radius_outer / r) ** 2 * (1 + nu_analy))
    )

    r_x = np.hstack((r.reshape(-1, 1), y.reshape(-1, 1)))
    output = model.predict(r_x)
    u_pred, v_pred = (
        output[:, 0:1] * characteristic_disp / characteristic_length,
        output[:, 1:2] * characteristic_disp / characteristic_length,
    )
    u_rad_pred = np.sqrt(u_pred**2 + v_pred**2)
    sigma_xx, sigma_yy, sigma_xy = (
        output[:, 2:3] * characteristic_stress,
        output[:, 3:4] * characteristic_stress,
        output[:, 4:5] * characteristic_stress,
    )
    ...
    plt.savefig("Lame_quarter_gmsh_nondim_mixed")
    plt.show()
...
file_path = os.path.join(
    os.getcwd(), "Lame_quarter_gmsh_nondimensionalized_newformulation"
)
