"""
Solves a hollow cylinder under internal pressure using gmsh geometry (Lame problem)

This example demonstrates:
- Creating custom 2D geometries with gmsh
- Applying traction boundary conditions
- Computing stresses in polar coordinates
...
quarter_circle_with_hole = CirclewithHole(
    center=[0, 0, 0],
    inner_radius=1,
    outer_radius=2,
    mesh_size=0.1,
...
def boundary_two_points(x, on_boundary):
    """Check whether a point satisfies the `boundary_two_points` boundary condition.
    ...
    Returns:
        bool: Result of the `boundary_two_points` evaluation.
    """
    return on_boundary and (
        np.logical_and(np.isclose(x[0], 1), np.isclose(x[1], 0))
        or np.logical_and(np.isclose(x[0], 2), np.isclose(x[1], 0))
    )
...
bc3 = dde.DirichletBC(
    geom, lambda _: 0.0, boundary_two_points, component=1
)  # to avoid rotation
