...
from compsim_pinns.geometry.gmsh_models import Rectangle_4PointBending

# gmsh options
gmsh_options = {"General.Terminal": 1, "Mesh.Algorithm": 6}
# the start points and the size of 4 points are given
region_size_dict = {
    "r1": {"start": 0.25, "increment": 0.1},
    "r2": {"start": 2.65, "increment": 0.1},
    "r3": {"start": 1.1, "increment": 0.1},
    "r4": {"start": 1.8, "increment": 0.1},
}

# generate the block, gmsh model and geometry for further calculations
block_2d = Rectangle_4PointBending(
    l_beam=3,
    h_beam=0.3,
...
def neumann_y(x, y, X):
    ...
    return zero_neumman_plane_stress_y(x, y, X) + pressure


def dirichlet_region(x, on_boundary):
    """Find the points on where the BCs are applied"""
    region_1 = np.logical_and(
        x[0] >= region_size_dict["r1"]["start"],
        x[0] <= (region_size_dict["r1"]["start"] + region_size_dict["r1"]["increment"]),
    )
    region_2 = np.logical_and(
        x[0] >= region_size_dict["r2"]["start"],
        x[0] <= (region_size_dict["r2"]["start"] + region_size_dict["r2"]["increment"]),
    )
...
def force_region(x, on_boundary):
    ...
    region_3 = np.logical_and(
        x[0] >= region_size_dict["r3"]["start"],
        x[0] <= (region_size_dict["r3"]["start"] + region_size_dict["r3"]["increment"]),
    )
    region_4 = np.logical_and(
        x[0] >= region_size_dict["r4"]["start"],
        x[0] <= (region_size_dict["r4"]["start"] + region_size_dict["r4"]["increment"]),
    )
...
def free_regions(x, on_boundary):
    ...
    return on_boundary and ~bottom_points and ~top_points
