...
def neumann_y_top(x, y, X):
    ...
    return sigma_yy_n_y + assign_pressure[assign_func_type] * (
        assigned_func(load_center, x_loc, assign_deviation_top[assign_func_type])
        + assigned_func(-load_center, x_loc, assign_deviation_top[assign_func_type])
    )
...
def neumann_y_bottom(x, y, X):
    ...
    return sigma_yy_n_y + assign_pressure[assign_func_type] * (
        assigned_func(disp_center, x_loc, assign_deviation_bottom[assign_func_type])
        + assigned_func(-disp_center, x_loc, assign_deviation_bottom[assign_func_type])
    )
...
def top_bottom(x, on_boundary):
    """Find the points on top and bottom"""
    top_mask = np.isclose(x[1], coord_right_corner[1])
    bottom_mask = np.isclose(x[1], coord_left_corner[1])
...
def strain_xx(x, y, X):
    """Calculate the xx component of the strain tensor"""
    return dde.grad.jacobian(y, x, i=0, j=0)


# Experimental Data from Johannes
xChannel2 = np.load(
    Path(__file__).parent.parent.parent.absolute() / "train_data/xChannel2.npy"
)
measured_strains_2 = np.load(
    Path(__file__).parent.parent.parent.absolute()
    / "train_data/strainsInitialChannel2.npy"
)
...
xchannel2_strain_combined = np.hstack(
    (
        xChannel2.reshape(-1, 1),
        0.12 * np.ones(xChannel2.shape).reshape(-1, 1),
        measured_strains_2.reshape(-1, 1),
    )
)
np.random.seed(12)
np.random.shuffle(xchannel2_strain_combined)

ex_data_xy_strain = xchannel2_strain_combined[
    :200, 0:2
]  # np.vstack((xchannel2_strain_combined[:100,0:2],xchannel1_strain_combined[:100,0:2]))
measured_strains_sliced = xchannel2_strain_combined[
    :200, 2:3
]  # np.vstack((xchannel2_strain_combined[:100,2:3], xchannel1_strain_combined[:100,2:3]))

observe_eps_xx = dde.PointSetOperatorBC(
    ex_data_xy_strain, measured_strains_sliced, strain_xx
)
...
def output_transform(x, y):
    ...
    return tf.concat(
        [
            u * x_loc,
            v,
            sigma_xx * (x_loc - l / 2) * (x_loc + l / 2),
            sigma_yy,
            sigma_xy * (x_loc - l / 2) * (x_loc + l / 2),
        ],
        axis=1,
    )
