...
region_size_dict = {
    "r1": {"center": -disp_center, "deviation": disp_deviation},
    "r2": {"center": disp_center, "deviation": disp_deviation},
    "r3": {"center": -load_center, "deviation": load_deviation},
    "r4": {"center": load_center, "deviation": load_deviation},
}

# generate the block, gmsh model and geometry for further calculations
block_2d = Rectangle_4PointBendingCentered(
    coord_left_corner=coord_left_corner,
    coord_right_corner=coord_right_corner,
    region_size_dict=region_size_dict,
    mesh_size=0.08,
    refine_factor=30,
...
def heaviside_subtracted(x_center, x_loc, deviation):
    ...
    """
    return np.heaviside(x_loc + (x_center + deviation), 1) - np.heaviside(
        x_loc + (x_center - deviation), 1
    )
...
def heaviside_subtracted_tf(x_center, x_loc, deviation):
    ...
    """
    return tf_original.experimental.numpy.heaviside(
        x_loc + (x_center + deviation), 1
    ) - tf_original.experimental.numpy.heaviside(x_loc + (x_center - deviation), 1)
...
factor = 5000
y_exp = exp_func(load_center, x_loc_upper, factor) + exp_func(
    -load_center, x_loc_upper, factor
)

# step function
y_step = heaviside_subtracted(
    load_center, x_loc_upper, load_deviation
) + heaviside_subtracted(-load_center, x_loc_upper, load_deviation)

# B-spline function
y_bsp = b_spline(load_center, x_loc_upper, load_deviation) + b_spline(
    -load_center, x_loc_upper, load_deviation
)
...
pressure_step = 1
pressure_exp = np.trapz(y_step, x_loc_upper) / np.trapz(
    y_exp, x_loc_upper
)  # max height of exponential func.
pressure_bsp = np.trapz(y_step, x_loc_upper) / np.trapz(
    y_bsp, x_loc_upper
)  # max height of Bspline.

print(
    f"The expected area: 0.02, step func. area: {pressure_step * np.trapz(y_step, x_loc_upper)}"
)
print(
    f"The expected area: 0.02, exp. func. area: {pressure_exp * np.trapz(y_exp, x_loc_upper)}"
)
print(
    f"The expected area: 0.02, B-spline area  : {pressure_bsp * np.trapz(y_bsp, x_loc_upper)}"
)
...
if plot_loads:
    ...
    for ax in np.atleast_1d(axs).flat:
        ax.set_xticks(np.arange(-1.5, 1.5, 0.1))
    plt.show()
...
def neumann_y(x, y, X):
    ...
    if (
        assign_func_type == "bspline"
    ):  # B-spline is a bit special since it is not the tensorflow function, X instead of x is used (X and x are identical but X is np.array and x in tf.Tensor)
        x_loc = X[:, 0:1][cond]
        y_np = assign_pressure[assign_func_type] * (
            assigned_func(load_center, x_loc, assign_deviation[assign_func_type])
            + assigned_func(-load_center, x_loc, assign_deviation[assign_func_type])
        )
        y_tf = tf.convert_to_tensor(y_np, dtype=config.real(tf))
        return sigma_yy_n_y + y_tf
    else:
        return sigma_yy_n_y + assign_pressure[assign_func_type] * (
            assigned_func(load_center, x_loc, assign_deviation[assign_func_type])
            + assigned_func(-load_center, x_loc, assign_deviation[assign_func_type])
        )
...
def dirichlet_region(x, on_boundary):
    ...
    region_1 = np.logical_and(
        x[0]
        >= (region_size_dict["r1"]["center"] - region_size_dict["r1"]["deviation"]),
        x[0]
        <= (region_size_dict["r1"]["center"] + region_size_dict["r1"]["deviation"]),
    )
    region_2 = np.logical_and(
        x[0]
        >= (region_size_dict["r2"]["center"] - region_size_dict["r2"]["deviation"]),
        x[0]
        <= (region_size_dict["r2"]["center"] + region_size_dict["r2"]["deviation"]),
    )
...
def output_transform(x, y):
    ...
    u = y[:, 0:1]
    v = y[:, 1:2]
    return tf.concat([u * 1e-2, v * 1e-3], axis=1)
...
combined_strain = tuple(
    np.vstack(
        (
            np.array(eps_xx.flatten().tolist()),
            np.array(eps_yy.flatten().tolist()),
            np.array(eps_xy.flatten().tolist()),
        )
    )
)
...
file_path = os.path.join(
    os.getcwd(), f"Beam2D_4point_compare_loading_{assign_func_type}"
)
