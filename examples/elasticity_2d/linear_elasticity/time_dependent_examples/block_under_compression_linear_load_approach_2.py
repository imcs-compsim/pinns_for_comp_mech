...
from compsim_pinns.elasticity.elasticity_utils import (
    get_tractions_mixed_2d_spacetime,
...
block_2d = Block_2D(
    ...
    coord_right_corner=[length, height],
    mesh_size=0.05,
...
export_normals_tangentials_to_vtk(
    ...
    file_name="square_boundary_normals",
)
...
def apply_velocity_in_x(x, y, X):
    ...
    return du_x_t - pressure / youngs_modulus * nu * (1 + nu) * x_loc
...
def apply_velocity_in_y(x, y, X):
    ...
    y_loc = x[:, 1:2]
    return du_y_t + pressure / youngs_modulus * (1 - nu**2) * y_loc
...
def output_transform(x, y):
    ...
    return bkd.concat(
        [
            u
            * (x_at_0)
            * t_loc,  # u_x is 0 at x=0 (Dirichlet BC) + u_x = 0 at t=0 (Initial BC)
            v
            * (y_at_0)
            * t_loc,  # u_y is 0 at y=0 (Dirichlet BC) + u_y = 0 at t=0 (Initial BC)
            sigma_xx * (x_at_l),
            sigma_yy,
