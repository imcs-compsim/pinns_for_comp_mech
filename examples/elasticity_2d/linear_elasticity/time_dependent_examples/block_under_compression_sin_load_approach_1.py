...
from compsim_pinns.elasticity.elasticity_utils import (
    get_tractions_mixed_2d_time,
    pde_mixed_plane_strain_time_dependent,
    problem_parameters,
)

# This is for visualization
from compsim_pinns.geometry.custom_geometry import GmshGeometry2D, GmshGeometry3D
from compsim_pinns.geometry.geometry_utils import calculate_boundary_normals_3D
from compsim_pinns.geometry.gmsh_models import Block_2D, Block_3D_hex
...
export_normals_tangentials_to_vtk(
    ...
    file_name="block_boundary_normals",
)


def analytical_solution(x, t):
    """Compute analytical solution for this example setup.
    ...
    Returns:
        Any: Computed value returned by `analytical_solution`.
    """
    x_loc = x[:, 0:1]
    y_loc = x[:, 1:2]
...
def body_force(x):
    """Compute body force for this example setup.

    Args:
        x: Input coordinates used to evaluate the function.

    Returns:
        Any: Computed value returned by `body_force`.
    """
    x_loc = x[:, 0:1]
    y_loc = x[:, 1:2]
    t_loc = x[:, 2:3]
...
def boundary_top(x, on_boundary):
    """Check whether a point satisfies the `boundary_top` boundary condition.
    ...
    Returns:
        bool: Result of the `boundary_top` evaluation.
    """
...
def apply_pressure_y_top(x, y, X):
    """Compute apply pressure y top for this example setup.
    ...
    Returns:
        Any: Computed value returned by `apply_pressure_y_top`.
    """
...
def output_transform(x, y):
    ...
    return bkd.concat(
        [
            u
            * (x_at_0)
            * t_at_0,  # u_x is 0 at x=0 (Dirichlet BC) + u_x = 0 at t=0 (Initial BC)
            v
            * (y_at_0)
            * t_at_0,  # u_y is 0 at y=0 (Dirichlet BC) + u_y = 0 at t=0 (Initial BC)
            sigma_xx * (x_at_l),
            sigma_yy,
...
for i, current_time in enumerate(time_element):
    ...
    X_spacetime = np.hstack(
        (X, np.full([len(X), 1], current_time, dtype=config.real(np)))
    )
    ...
    rel_err_l2_disp = np.linalg.norm(
        displacement_a.flatten() - displacement_pred.flatten()
    ) / np.linalg.norm(displacement_a)
    print(f"Relative L2 error for displacement at t={current_time}: ", rel_err_l2_disp)

    sigma = np.vstack(
        (sigma_xx.reshape(-1, 1), sigma_yy.reshape(-1, 1), sigma_xy.reshape(-1, 1))
    )
    sigma_pred = np.vstack(
        (
            sigma_xx_pred.reshape(-1, 1),
            sigma_yy_pred.reshape(-1, 1),
            sigma_xy_pred.reshape(-1, 1),
        )
    )
    ...
    axs[2].plot(
        x.flatten(), sigma_yy.flatten(), label=f"t = {current_time}", color=color
    )
    axs[2].plot(
        x.flatten(),
