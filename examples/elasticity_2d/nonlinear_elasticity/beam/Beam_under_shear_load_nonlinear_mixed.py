...
from compsim_pinns.hyperelasticity.hyperelasticity_utils import compute_elastic_properties, cauchy_stress_2D, first_piola_stress_tensor_2D, deformation_gradient_2D
from compsim_pinns.elasticity.elasticity_utils import calculate_boundary_normals, stress_to_traction_2d
...
def boundary_free_surf(x, on_boundary):
    return on_boundary and (np.isclose(x[1],h_beam/2) or np.isclose(x[1],-h_beam/2)) and not (np.isclose(x[0],l_beam) or np.isclose(x[0],0))
...
bc2 = dde.DirichletBC(geom, lambda _: -shear_load, boundary_right, component=4) # Pyx = -t
...
def momentum_nonlinear_2d_mixed(x, y):
    # governing equation (dependent of ux, uy)
    ...
    # governing equation (dependent from Pxx, Pxy, Pyx, Pyy)
    ...
    # calculate strain terms (kinematics) from displacements
    ...
    return [momentum_x, momentum_y, residuum_Pxx, residuum_Pxy, residuum_Pyx, residuum_Pyy]
...
def output_transform(x, y):
    ...
    return bkd.concat([rel_length_left * h_beam * u_x,
                       rel_length_left * h_beam * u_y,
                       rel_length_right * e_modul * P_xx,
                       rel_height_pos * rel_height_neg * e_modul * P_xy,
                       - shear_load + rel_length_right * e_modul * P_yx,
                       rel_height_pos * rel_height_neg * e_modul * P_yy], axis=1)
...
simulation_case = f"Beam_under_shear_load_nonlinear_incremental_mixed"
...
# Weights
w_pde_1,w_pde_2,w_pde_3,w_pde_4,w_pde_5,w_pde_6 = 1E0,1E0,1E0,1E0,1E0,1E0
w_neumman_right_shear_x, w_neumman_right_shear_y, w_neumman_free_surface_x, w_neumman_free_surface_y = 1E0,1E0,1E0,1E0
loss_weights = [w_pde_1,w_pde_2,w_pde_3,w_pde_4,w_pde_5,w_pde_6]#,w_neumman_right_shear_x, w_neumman_right_shear_y, w_neumman_free_surface_x, w_neumman_free_surface_y]
...
def compute_displacement_sigma_nonlinear_mixed(model,points):
    ...
    # sigma_yx = 1 / det_f * (output[:,4:5] * f_xx + output[:,5:6] * f_xy)
...
for i in range(steps):
    ...
    model.compile("adam", lr=learning_rate_adam, loss_weights=loss_weights)#, decay=("exponential", exponential_decay))
    time_dict["simulation_compiling_adam"].append(time.time())
    ...
    node_tags, node_coords, _ = geom.gmsh_model.mesh.getNodes(2, -1, includeBoundary=True)
    ...
    _, _, _, n_nodes_per_cell, _, _ = geom.gmsh_model.mesh.getElementProperties(element_type)
    elements = node_tags_per_element[0].reshape(-1, n_nodes_per_cell) - 1
    vtk_cell_type_map = {2: 5, 3: 9}
    cell_types = np.full(elements.shape[0], vtk_cell_type_map[element_type], dtype=np.uint8)
    ...
    displacement_pred, cauchy_stress_pred = compute_displacement_sigma_nonlinear_mixed(model,points)
    ...
    if any(abs(shear_load * 1000 - i) <= 1e-12 for i in range(1, 11)):
        ...
        displacement_pred_on_fem_mesh, sigma_pred_on_fem_mesh = compute_displacement_sigma_nonlinear_mixed(model,points_fem[:,0:2])
        cauchy_stress_pred_on_fem_mesh = np.column_stack((sigma_pred_on_fem_mesh[:,0], sigma_pred_on_fem_mesh[:,1], np.zeros_like(sigma_pred_on_fem_mesh[:,0]), sigma_pred_on_fem_mesh[:,2], np.zeros_like(sigma_pred_on_fem_mesh[:,0]), np.zeros_like(sigma_pred_on_fem_mesh[:,0])))
        tensor_cauchy_stress_pred_on_fem_mesh = np.transpose(np.array([[sigma_pred_on_fem_mesh[:,0], sigma_pred_on_fem_mesh[:,2]],
                                                                       [sigma_pred_on_fem_mesh[:,2], sigma_pred_on_fem_mesh[:,1]]]),(2,0,1))
    ...
    trajectory_edge_points[i+1,:,:] = coords_edge_points + model.predict(coords_edge_points)[:,0:2]
    trajectory_corners[i+1,:,:] = coords_corners + model.predict(coords_corners)[:,0:2]
