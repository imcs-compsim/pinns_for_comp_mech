...
In this script, four different methods are described to enforce the Karush-Kuhn-Tucker inequalities (gn>=0, On<=0 and gn*On=0):
 ...
 - A complementarity function: On-max(0, On-c*gn) used often in computational contact mechanics
...
from compsim_pinns.contact_mech.contact_utils import (
    ...
    zero_complementarity_function_based_fischer_burmeister,
...
if method_name == "KKT_inequality_sign":
    ...
    bc_zero_complementarity = dde.OperatorBC(
        geom, zero_complementarity_function_based_fischer_burmeister, boundary_contact
    )
    bcs_ = [bc_zero_complementarity, bc_zero_tangential_traction]
