"""
Hertzian normal contact problem with Fischer-Burmeister complementarity conditions.

This module solves a 2D frictionless normal contact problem on a quarter disc geometry
using Physics-Informed Neural Networks (PINNs) with DeepXDE. The solution enforces
Karush-Kuhn-Tucker (KKT) contact conditions via the Fischer-Burmeister equation.

The trained model predictions are compared against FEM reference results and exported
to VTK format for visualization, including displacement, Cauchy stress, polar stress
components, and corresponding error metrics.
...
model_path = (
    ...
    + "/pretrained_models/elasticity_2d/hertzian/pure/hertzian"
)
...
file_path = os.path.join(
    os.getcwd(), "Hertzian_normal_contact_fischer_burmeister_w_e3_255"
)
