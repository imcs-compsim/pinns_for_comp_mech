"""
Physics-Informed Neural Network (PINN) for 2D Hertzian Normal Contact Problem.

Solves the frictionless contact mechanics problem using PINNs with the Fischer-Burmeister
complementarity condition to enforce Karush-Kuhn-Tucker (KKT) contact constraints.
The solution is validated against FEM results and visualized using VTK output.

Key features:
- Quarter-disc geometry with mesh generation via Gmsh
- Mixed plane strain elasticity formulation
- Frictionless contact boundary conditions
- Fischer-Burmeister equation for KKT condition enforcement
- Hard boundary condition implementation via output transformation
- Comparative visualization of PINN vs FEM predictions and error analysis
