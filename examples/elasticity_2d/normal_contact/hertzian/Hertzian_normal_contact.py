"""
2D Hertzian Normal Contact Problem using Physics-Informed Neural Networks (PINNs).

This module solves a frictionless contact problem between a rigid cylinder and an elastic
quarter-disc domain using PINNs. It enforces Karush-Kuhn-Tucker (KKT) conditions for contact
mechanics, including gap non-negativity, normal traction negativity, zero tangential traction,
and complementarity conditions. Results are compared against FEM solutions and exported to VTK format.
...
def negative_normal_traction(x, y, X):
    """
    Enforces normal part of contact traction (On) to be negative.
    """
    Tx, Ty, On, Tt = calculate_traction(x, y, X)

    # If On is positive, it will create contributions to overall loss. Aims is to get negative normal traction
    return (1.0 + torch.sign(On)) * On
...
bc_positive_normal_gap = dde.OperatorBC(
    geom, positive_normal_gap, boundary_circle_contact
)
bc_negative_normal_traction = dde.OperatorBC(
    geom, negative_normal_traction, boundary_circle_contact
)
bc_zero_tangential_traction = dde.OperatorBC(
    geom, zero_tangential_traction, boundary_circle_contact
)
bc_zero_complimentary = dde.OperatorBC(
    geom, zero_complimentary, boundary_circle_contact
)
