"""
Solves Hertzian normal contact example including simulation data from 4C.
...
from compsim_pinns.elasticity.elasticity_utils import (
    calculate_traction_mixed_formulation,
    pde_mixed_plane_strain,
    problem_parameters,
    zero_neumann_x_mixed_formulation,
    zero_neumann_y_mixed_formulation,
)
...
nu, lame, shear, youngs_modulus = (
    problem_parameters()
)  # with dimensions, will be used for analytical solution
# This will lead to youngs_modulus=200 and nu=0.3
...
def zero_fischer_burmeister(x, y, X):
    ...
    Tx, Ty, On, Tt = calculate_traction_mixed_formulation(x, y, X)
    ...
    return (
        ...
        - torch.sqrt(
            torch.maximum(
                a**2 + b**2, torch.tensor(1e-9, dtype=a.dtype, device=a.device)
            )
        )
    )
...
def boundary_circle_not_contact(x, on_boundary):
    """Check whether a point satisfies the `boundary_circle_not_contact` boundary condition.
    ...
    Returns:
        bool: Result of the `boundary_circle_not_contact` evaluation.
    """
    return (
        on_boundary
        and np.isclose(np.linalg.norm(x - center, axis=-1), radius)
        and (x[0] < x_loc_partition)
    )


def boundary_circle_contact(x, on_boundary):
    """Check whether a point satisfies the `boundary_circle_contact` boundary condition.
    ...
    Returns:
        bool: Result of the `boundary_circle_contact` evaluation.
    """
    return (
        on_boundary
        and np.isclose(np.linalg.norm(x - center, axis=-1), radius)
...
# Neumann BC
bc_zero_traction_x = dde.OperatorBC(
    geom, zero_neumann_x_mixed_formulation, boundary_circle_not_contact
)
bc_zero_traction_y = dde.OperatorBC(
    geom, zero_neumann_y_mixed_formulation, boundary_circle_not_contact
)

# Contact BC
bc_zero_fischer_burmeister = dde.OperatorBC(
    geom, zero_fischer_burmeister, boundary_circle_contact
)
bc_zero_tangential_traction = dde.OperatorBC(
    geom, zero_tangential_traction, boundary_circle_contact
)
...
if add_external_data:
    ...
    #
    ex_data_xy = np.vstack(
        (
            node_coords_xy[on_boundary_][:n_boundary],
            node_coords_xy[~on_boundary_][:n_domain],
        )
    )
    ex_data_disp = np.vstack(
        (
            displacement_fem[on_boundary_][:n_boundary],
            displacement_fem[~on_boundary_][:n_domain],
        )
    )
    ex_data_stress = np.vstack(
...
def output_transform(x, y):
    ...
    return torch.cat(
        [
            ...
            v / youngs_modulus,
            sigma_xx,
            ext_traction + sigma_yy * (-y_loc),
            sigma_xy * (x_loc) * (y_loc),
        ],
        axis=1,
    )
...
sigma_xx_pred, sigma_yy_pred, sigma_xy_pred = (
    output[:, 2:3],
    output[:, 3:4],
    output[:, 4:5],
)
sigma_rr_pred, sigma_theta_pred, sigma_rtheta_pred = polar_transformation_2d(
    sigma_xx_pred, sigma_yy_pred, sigma_xy_pred, X
)

combined_disp_pred = tuple(
    np.vstack(
        (
...
combined_stress_pred = tuple(
    np.vstack(
        (
            np.array(sigma_xx_pred.flatten().tolist()),
            np.array(sigma_yy_pred.flatten().tolist()),
            np.array(sigma_xy_pred.flatten().tolist()),
        )
    )
)
combined_stress_polar_pred = tuple(
    np.vstack(
        (
            np.array(sigma_rr_pred.tolist()),
            np.array(sigma_theta_pred.tolist()),
            np.array(sigma_rtheta_pred.tolist()),
        )
    )
)
...
sigma_xx_fem, sigma_yy_fem, sigma_xy_fem = (
    stress_fem[:, 0:1],
    stress_fem[:, 1:2],
    stress_fem[:, 2:3],
)
sigma_rr_fem, sigma_theta_fem, sigma_rtheta_fem = polar_transformation_2d(
    sigma_xx_fem, sigma_yy_fem, sigma_xy_fem, X
)

combined_disp_fem = tuple(
    np.vstack(
        (np.array(u_fem.tolist()), np.array(v_fem.tolist()), np.zeros(u_fem.shape[0]))
    )
)
combined_stress_fem = tuple(
    np.vstack(
        (
            np.array(sigma_xx_fem.flatten().tolist()),
            np.array(sigma_yy_fem.flatten().tolist()),
            np.array(sigma_xy_fem.flatten().tolist()),
        )
    )
)
combined_stress_polar_fem = tuple(
    np.vstack(
        (
            np.array(sigma_rr_fem.tolist()),
            np.array(sigma_theta_fem.tolist()),
            np.array(sigma_rtheta_fem.tolist()),
        )
    )
)
...
combined_error_disp = tuple(
    np.vstack((error_disp_x, error_disp_y, np.zeros(error_disp_x.shape[0])))
)

error_stress_x = abs(
    np.array(sigma_xx_pred.flatten().tolist()) - sigma_xx_fem.flatten()
)
error_stress_y = abs(
    np.array(sigma_yy_pred.flatten().tolist()) - sigma_yy_fem.flatten()
)
error_stress_xy = abs(
    np.array(sigma_xy_pred.flatten().tolist()) - sigma_xy_fem.flatten()
)
combined_error_stress = tuple(
    np.vstack((error_stress_x, error_stress_y, error_stress_xy))
)

error_polar_stress_x = abs(
    np.array(sigma_rr_pred.flatten().tolist()) - sigma_rr_fem.flatten()
)
error_polar_stress_y = abs(
    np.array(sigma_theta_pred.flatten().tolist()) - sigma_theta_fem.flatten()
)
error_polar_stress_xy = abs(
    np.array(sigma_rtheta_pred.flatten().tolist()) - sigma_rtheta_fem.flatten()
)
...
unstructuredGridToVTK(
    ...
    cell_types,
    pointData={
        "displacement_pred": combined_disp_pred,
...
x_contact_lim = 2 * np.sqrt(
    2 * radius**2 * abs(ext_traction) * (1 - nu**2) / (youngs_modulus * np.pi)
)
x_contact_cond = np.logical_and(
    np.isclose(np.linalg.norm(X - center, axis=-1), radius), X[:, 0] > -x_contact_lim
)
...
pc_analytical = (
    ...
    * radius
    * abs(ext_traction)
    / (np.pi * x_contact_lim**2)
    * np.sqrt(x_contact_lim**2 - node_coords_x_contact**2)
)
