...
boundary_selection_map = [{"boundary_function" : on_bottom_contact, "tag" : "on_bottom_contact"}]
...
def output_transform(x, y):
    ...
    u_out = (height - y_loc) * u
    v_out = v + y_loc / height * displacement_buckling
    w_out = (height - y_loc) * w
...
# Model parameters
steps = 1
max_displacement_buckling = -0.002 # [m]
model_path = str(Path(__file__).parent)
simulation_case = f"3d_cylinder_buckling_contact_displacement_incremental"
...
for i in range(steps):
    ...
    # if (displacement_buckling % 15 == 0) & (displacement_buckling <= max_displacement_buckling):
    #     fem_path = str(Path(__file__).parent.parent.parent.parent) + "/fem_references/paper-epinn-data-reference"
    #     fem_reference = pv.read(fem_path+f"/3d_torsion_prism/fem_reference_3d_block_max_displacement_buckling_{int(displacement_buckling):03}.vtu")
    ...
    #     file_path_fem_compare = os.path.join(model_path, f"{simulation_case}_fem_compare_{int(displacement_buckling):03}")
    #     fem_reference.save(f"{file_path_fem_compare}.vtu")

    #     # Look at results at gauss points
    #     fem_gp_reference = pv.read(fem_path+f"/3d_torsion_prism/fem_reference_3d_block_max_displacement_buckling_gp_info_{int(displacement_buckling):03}.vtu")
    ...
    file_path = os.path.join(model_path, f"{simulation_case}_{int(abs(displacement_buckling*1E3)):03}")
...
ax1.plot(losshistory.steps, [abs(loss[1]) for loss in losshistory.loss_train], label="Contact work", marker="x")
