"""
3D Block Torsion Analysis with Nonlinear Displacement Incremental Training using DeepXDE and Adam Optimizer.

This module implements a physics-informed neural network (PINN) solver for simulating nonlinear
torsion of a 3D rectangular block using the deep energy method. The simulation applies incremental
rotational displacement at one end (x=L) while keeping the other end (x=0) fixed, progressively
increasing the rotation angle from 0 to 150 degrees over multiple training steps.

Key Features:
- Uses DeepXDE framework with deep energy formulation
- Neo-Hookean hyperelastic material model
- Gmsh-based 3D hexahedral mesh generation
- Gaussian quadrature integration for energy computation
- Output transform to enforce boundary conditions via smooth displacement blending
- Adam optimizer for neural network training
- VTK export for visualization of displacement and stress fields (Cauchy and Piola stresses)

The network predicts displacement fields (u, v, w) that satisfy the energy minimization principle,
from which stresses are computed and exported for each incremental rotation step.
...
def points_at_back(x, on_boundary):
    """Identify points on the back boundary of a domain.

    Filters for points that are located at the back surface (x=0) and are on the boundary.

    Args:
        x (ndarray): Coordinate array where x[0] represents the x-coordinate values.
        on_boundary (bool): Boolean flag indicating whether the points are on the boundary.

    Returns:
        bool: True if the point is on the boundary and has x-coordinate close to 0, False otherwise.
    """
...
def output_transform(x, y):
    """Transform network output to satisfy boundary conditions for 3D block torsion.

    Applies displacement boundary conditions where the block is twisted by a rotation angle
    at x = L while being fixed at x = 0. The output is a smooth blend between the prescribed
    displacement at the boundary and the network prediction.

    Args:
        x (Tensor): Input coordinates of shape (batch_size, 3) containing [x, y, z] locations.
        y (Tensor): Network output of shape (batch_size, 3) containing raw displacement
                   predictions [u, v, w] for each coordinate.

    Returns:
        Tensor: Transformed displacement field of shape (batch_size, 3) with boundary
               conditions applied, containing [u_out, v_out, w_out].
    """
...
for i in range(steps):
    theta_deg = 150 / steps * (i + 1)
    model.compile("adam", lr=0.001)
    losshistory, train_state = model.train(epochs=500, display_every=100)
    ...
    combined_shear_stress_pred = np.vstack(
        (sigma_xy_pred, sigma_yz_pred, sigma_xz_pred)
    )
    ...
    file_path = os.path.join(os.getcwd(), f"deep_energy_3d_block_torsion_nonlinear_{i}")
