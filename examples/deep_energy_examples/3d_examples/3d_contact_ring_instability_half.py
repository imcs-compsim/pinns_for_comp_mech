...
quarter_circle_with_hole = RingHalf3D(center=center, inner_radius=0.9, outer_radius=1, mesh_size=0.025, num_elements=5)
...
quad_rule_boundary_integral = GaussQuadratureRule(rule_name="gauss_legendre", dimension=boundary_dimension, ngp=3) # gauss_legendre gauss_labotto
...
def potential_energy(X, 
    ...
    internal_energy = global_element_weights_t[:,0:1]*global_element_weights_t[:,1:2]*global_element_weights_t[:,2:3]*(internal_energy_density[beg_pde:beg_boundary])*jacobian_t
    ...
    contact_work = global_weights_boundary_t[:,0:1][cond]*global_weights_boundary_t[:,1:2][cond]*(contact_force_density)*jacobian_boundary_t[cond]
...
def output_transform(x, y):
    ...
    z_loc = x[:, 2:3]
    # tf.math.abs(x_loc)
    return bkd.concat([u*(-y_loc)*(x_loc*x_loc)/e_modul, 
...
restore_model = False
model_path = str(Path(__file__).parent.parent.parent)+f"/pretrained_models/deep_energy_examples/3d_contact_ring_instability/3d_contact_ring_instability_half_nonlinear"
