"""3D Contact Ring Instability Example using Deep Energy Method.

This example demonstrates a 3D ring (half geometry) undergoing contact with a rigid
surface using the energy-based PINNs. The simulation models the hyperelastic
deformation of a Neo-Hookean material under displacement-controlled loading.

Key features:
- 3D half-ring geometry with inner and outer radii
- Neo-Hookean hyperelasticity constitutive model
- Contact mechanics with penalty method
- Deep Energy approach for computing strain energy
- Gaussian quadrature for numerical integration
- Output saved in VTU format with displacement and Cauchy stress fields

The model is trained using Adam optimizer followed by L-BFGS optimization to predict
displacements and stresses throughout the domain while satisfying contact constraints.
"""
...
from compsim_pinns.geometry.gmsh_models import RingHalf3D
...
quarter_circle_with_hole = RingHalf3D(
    center=center, inner_radius=0.9, outer_radius=1, mesh_size=0.025, num_elements=5
)
...
def output_transform(x, y):
    ...
    return bkd.concat(
        [
            u * (-y_loc) * (x_loc * x_loc) / youngs_modulus,
...
model_path = (
    ...
    + f"/pretrained_models/deep_energy_examples/3d_contact_ring_instability/3d_contact_ring_instability_half_nonlinear"
)
...
grid.save("deep_energy_3d_contact_ring_instability_half.vtu")
