"""
3D Block Torsion Analysis using Physics-Informed Neural Networks (PINNs)

This module implements a nonlinear torsion simulation for a 3D hexahedral block using
the Deep Energy method with L-BFGS optimization. The solution employs incremental loading
with a gradually increasing rotation angle applied at the free end (x=L) while the
opposite end (x=0) is fixed.

Key Features:
- 3D block geometry meshed with hexahedral elements via Gmsh
- Neo-Hookean hyperelastic material model
- Deep Energy formulation based on strain energy minimization
- Incremental displacement loading via output transform
- L-BFGS optimizer for improved convergence
- VTK output for post-processing (displacements, Cauchy stress, First Piola-Kirchhoff stress)
- Gaussian quadrature integration over domain and boundary elements

Materials and Parameters:
- Young's modulus: 1.33
- Poisson's ratio: 0.33
- Applied rotation: 150 degrees (incremental over training epochs)

Output:
- Displacement field (u, v, w)
- Cauchy stress tensor components
- First Piola-Kirchhoff stress tensor components
- Results exported to VTK format for visualization
...
def output_transform(x, y):
    ...
    if model.data.current_epoch is not None:
        ...
        theta_deg_chunk = current_epoch / epochs * theta_deg
    else:
        theta_deg_chunk = 0
...
file_path = os.path.join(
    os.getcwd(), "3d_block_torsion_nonlinear_displacement_incremental_lbfgs_smooth"
)
epoch_tracker = EpochTracker()
model_saver_incremental = SaveModelVTU(period=200, filename=file_path)

LBFGS_options["iter_per_step"] = 1  # int(epochs/steps)
LBFGS_options["maxiter"] = epochs

model.compile("L-BFGS")
losshistory, train_state = model.train(
    epochs=epochs, callbacks=[epoch_tracker], display_every=1
)
...
file_path = os.path.join(
    os.getcwd(), "deep_energy_3d_block_torsion_nonlinear_incremental_lbfgs"
)
