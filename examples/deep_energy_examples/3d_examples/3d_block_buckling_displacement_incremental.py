"""
3D Block Buckling Under Incremental Displacement Loading
==================================================

This example solves a three-dimensional block buckling problem under
incrementally prescribed axial displacement with the Energy-based PINN (EPINN).
...
buckling displacement is increased in load steps. The model is trained with
Adam, with an optional relaxation step using Adam followed by L-BFGS.
...
for i in range(steps):
    ...
    file_path = os.path.join(
        model_path, f"{simulation_case}_{int(abs(displacement_buckling)):02}"
    )
