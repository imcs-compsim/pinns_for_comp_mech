...
def output_transform(x, y):
    ...
    u_out = x_loc * (x_loc - length) * u + x_loc / length * displacement_buckling
...
# Model parameters
steps = 10
max_displacement_buckling = -1
model_path = str(Path(__file__).parent)
simulation_case = f"3d_block_buckling_displacement_incremental"
...
for i in range(steps):
    ...
    file_path = os.path.join(model_path, f"{simulation_case}_{int(abs(displacement_buckling)):02}")
