...
from compsim_pinns.geometry.gmsh_models import CooksCantilever
...
from compsim_pinns.hyperelasticity.hyperelasticity_utils import compute_elastic_properties, deformation_gradient_3D, matrix_determinant_3D, bkd_log
...
Cantilever = CooksCantilever(length=length, web_height=web_height, load_height=load_height, thickness=thickness, divisions=[n_elements_length, n_elements_height, n_elements_thickness])
...
boundary_selection_map = [{"boundary_function" : cantilever_end, "tag" : "cantilever_end"}]
...
def strain_energy_neo_hookean_3d_modified(x, y):
    ...
    # Strain energy
    W = 0.5 * shear * (I_1 - 3) + lame / 4 * (det_f ** 2 - 1) - (lame / 2 + shear) * bkd_log(det_f)

    return W

def cauchy_stress_3D_modified(x, y):
    f_xx, f_yy, f_zz, f_xy, f_yx, f_xz, f_zx, f_yz, f_zy = deformation_gradient_3D(x, y)
    det_f = matrix_determinant_3D(f_xx, f_xy, f_xz,
                                  f_yx, f_yy, f_yz,
                                  f_zx, f_zy, f_zz)
...
def potential_energy(X, 
    ...
    internal_energy_density = strain_energy_neo_hookean_3d_modified(inputs, outputs)[beg_pde:beg_boundary]
...
def output_transform(x, y):
    ...
    u_out = x_loc * u
    v_out = x_loc * v
    w_out = x_loc * (thickness/2 - z_loc) * (thickness/2 + z_loc) * w # needed due to the weird normal implementation
...
if relaxation:
    ...
    model.compile("adam", lr=learning_rate_adam, loss_weights=[energy_scale, energy_scale])
    time_dict["relaxation_compiling"].append(time.time())
...
for i in range(steps):
    ...
    model.compile("adam", lr=learning_rate_adam, loss_weights=[energy_scale, energy_scale])#, decay=("exponential", exponential_decay))
    time_dict["simulation_compiling_adam"].append(time.time())
    ...
    sigma_xx, sigma_yy, sigma_zz, sigma_xy, sigma_yx, sigma_xz, sigma_zx, sigma_yz, sigma_zy = model.predict(points, operator=cauchy_stress_3D_modified)
    ...
    print(f"The predicted y-displacement in point A is {displacement_A[:,1][0] * 1E3} mm.")
