"""
Nonlinear 3D Cook's Cantilever Under Shear Loading
==================================================

This example solves a three-dimensional Cook's cantilever problem under shear
...
def cauchy_stress_3D_modified(x, y):
    ...
    Returns:
        tuple: Cauchy stress components in 3D.
    """
    f_xx, f_yy, f_zz, f_xy, f_yx, f_xz, f_zx, f_yz, f_zy = deformation_gradient_3D(x, y)
    det_f = matrix_determinant_3D(f_xx, f_xy, f_xz, f_yx, f_yy, f_yz, f_zx, f_zy, f_zz)
...
def output_transform(x, y):
    ...
    u_out = x_loc * u
    v_out = x_loc * v
    w_out = x_loc * (thickness / 2 - z_loc) * (thickness / 2 + z_loc) * w
