"""
Nonlinear 3D Block Torsion Under Incremental Rotation
==================================================
...
displacement is enforced through the output transform, and the prescribed angle
is increased in load steps. In each load step the model is trained using the
Adam optimizer with an exponentially decaying learning rate.

Results are exported as VTU files for post-processing, while nodal and
Gauss-point errors, probe-line data, timings, and loss information are also
saved.
"""
...
for i in range(steps):
    ...
    if (theta_deg % 15 == 0) & (theta_deg <= torsion_angle):
        ...
        fem_reference = pv.read(
            fem_path
            + f"/3d_torsion_prism/fem_reference_3d_block_torsion_angle_{compare_choice}_{int(theta_deg):03}.vtu"
        )
        ...
        # Compare fields at FEM Gauss points
        fem_gp_reference = pv.read(
            fem_path
            + f"/3d_torsion_prism/fem_reference_3d_block_torsion_angle_gp_info_{compare_choice}_{int(theta_deg):03}.vtu"
        )
        ...
        (
            ...
            sigma_yz_gp_pred,
            _,
        ) = model.predict(points_fem_gp, operator=cauchy_stress_3D)
        ...
        a = 1.0 / np.sqrt(3.0)
        gauss_points_ref = np.array(
            list(itertools.product([-a, a], repeat=3)), dtype=float
        )
        gauss_weights = np.ones(n_gp, dtype=float)

        def dN_dxi_hex8(xi, eta, zeta):
            """Compute HEX8 shape-function derivatives in reference coordinates.

            Args:
                xi: Reference coordinate xi.
                eta: Reference coordinate eta.
                zeta: Reference coordinate zeta.

            Returns:
                Any: Shape-function derivatives with respect to xi, eta, and zeta.
            """
            xi_i = ref_nodes[:, 0]
            eta_i = ref_nodes[:, 1]
            zeta_i = ref_nodes[:, 2]
        ...
        squared_error_disp_gp = np.sum(
            (displacement_pred_gp_e - displacement_fem_gp_e) ** 2, axis=2
        )
        ...
        squared_stress_gp = (
            cauchy_stress_fem_gp_e[:, :, 0] ** 2
            + cauchy_stress_fem_gp_e[:, :, 1] ** 2
            + cauchy_stress_fem_gp_e[:, :, 2] ** 2
            + 2.0 * cauchy_stress_fem_gp_e[:, :, 3] ** 2
            + 2.0 * cauchy_stress_fem_gp_e[:, :, 4] ** 2
            + 2.0 * cauchy_stress_fem_gp_e[:, :, 5] ** 2
        )
        l2_error_stress_gp = np.sum(
            squared_error_stress_gp * detJ * gauss_weights[None, :]
        )
        ...
        print(
            f"                                        nodal: {rel_vec_l2_stress_nodes}"
        )

        # Compare stresses at FEM element centers
        ...
        (
            ...
            _,
        ) = model.predict(element_centers_fem, operator=cauchy_stress_3D)
        ...
        print(
            f"                                      centers: {rel_vec_l2_stress_centers}"
        )

        # R2 scores for displacement and stress components on the nodal FEM mesh
        ...
        r2_stress = {
            label: r2_score(
                cauchy_stress_fem[:, i], cauchy_stress_pred_on_fem_mesh[:, i]
            )
            for i, label in enumerate(stress_labels)
        }

        print("\nComponent-wise R2 scores (nodal FEM mesh):")
        for k, v in r2_disp.items():
            print(f"{k:>8s}: {v: .8f}")
        for k, v in r2_stress.items():
            print(f"{k:>8s}: {v: .8f}")

    # Save probe lines for the final torsion angle
    if theta_deg == torsion_angle:
        ...
        (
            ...
            sigma_yz_yline,
            _,
        ) = model.predict(points_yline, operator=cauchy_stress_3D)
        stress_yline = np.column_stack(
            (
                sigma_xx_yline,
                sigma_yy_yline,
                sigma_zz_yline,
        ...
        (
            ...
            sigma_yz_zline,
            _,
        ) = model.predict(points_zline, operator=cauchy_stress_3D)
        stress_zline = np.column_stack(
            (
                sigma_xx_zline,
                sigma_yy_zline,
                sigma_zz_zline,
        ...
        np.savez(
            f"{model_path}/{simulation_case}_probe_lines.npz",
