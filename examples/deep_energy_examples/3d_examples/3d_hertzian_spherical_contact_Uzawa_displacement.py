"""
Hertzian Spherical Contact Under Incremental Displacement
==================================================

This example solves a three-dimensional Hertzian contact problem for an eighth
sphere under incrementally prescribed displacement with the Energy-based PINN
(EPINN).

It is modelled using a Neo-Hookean hyperelastic material. Contact is enforced
with an Uzawa-type augmented Lagrangian update, and the prescribed displacement
is increased in load steps. In each load step the model is trained over several
Uzawa loops using the Adam optimizer with an exponentially decaying learning
rate.

Results are exported as VTU files for post-processing, while non-penetration
violations, errors, timings, and loss information are also saved.
"""
...
def potential_energy(
    ...
    Args:
        ...
        lagrange_multipliers: Value for lagrange multipliers.
    ...
    cofF = J * FinvT
    current_normals = torch.nn.functional.normalize(
        torch.einsum("bij,bj->bi", FinvT, mapped_normal_boundary_t), dim=1
    )
    ...
    contact_force_density = (
        ...
        / (2 * epsilon)
        * (
            bkd.relu(lagrange_multipliers[cond] - epsilon * gap_n) ** 2
            - lagrange_multipliers[cond] ** 2
        )
    )
...
def output_transform(x, y):
    ...
    return bkd.concat(
        [
            ...
            -displacement_bc + v * top_surface / youngs_modulus,
...
simulation_case = f"3d_hertzian_spherical_contact_Uzawa_displacement"
...
# Define the Uzawa and early stopping callbacks
    ...
    for j in range(lagrange_loops):
        ...
        if j == 0:
            losshistory, train_state = model.train(
                ...
                display_every=100,
                callbacks=train_callbacks + [reset_lagrange],
            )
        else:
            losshistory, train_state = model.train(
                iterations=adam_iterations,
        ...
        # Return violations of non-penetration condition in y direction
        displaced_X_y = points[:, 1:2] + output[:, 1:2]
        displaced_X_y_violated = (
            displaced_X_y[displaced_X_y < projection_plane["y"]] - projection_plane["y"]
        )
        row = i * lagrange_loops + j
        if displaced_X_y_violated.size == 0:
            violations[row, :] = np.zeros((5,))
        else:
            violations[row, :] = [
                displaced_X_y_violated.size,
                -displaced_X_y_violated.mean(),
                displaced_X_y_violated.std(),
                -displaced_X_y_violated.min(),
                -displaced_X_y_violated.max(),
            ]
        print(
            f"The non-penetration condition is violated at {violations[row, 0]:.0f} points."
        )
        print(
            f"It is violated with a mean of {violations[row, 1]:.3e} "
            f"with a standard deviation of {violations[row, 2]:.3e}."
        )
        print(
            f"The maximal violation is {violations[row, 3]:.3e}, "
            f"the minimal is {violations[row, 4]:.3e}.\n"
        )

        # Compare the results with the FEM reference
        if (displacement_bc <= max_displacement_bc) and (
            min(displacement_bc % 0.008, 0.008 - displacement_bc % 0.008) < 1e-12
        ):
            fem_path = (
            ...
            fem_reference = pv.read(
                fem_path
                + f"/3d_contact_sphere/3d_sphere_contact_displacement_fem_reference_{int(displacement_bc * 1000):04}.vtu"
            )
            ...
            file_path_fem_compare = os.path.join(
                model_path,
                f"{simulation_case}_fem_compare_{int(displacement_bc * 1000):04}_uzawa_loop_{j + 1}",
            )
            fem_reference.save(f"{file_path_fem_compare}.vtu")

        file_path = os.path.join(
            model_path,
            f"{simulation_case}_{int(displacement_bc * 1000):04}_uzawa_loop_{j + 1}",
        )
    ...
    if steps > 1:
        for i in range(steps):
            for j in range(lagrange_loops):
                print(f"----------------------------------------------", file=text_file)
                print(
                    f"   Load step {(i + 1):2d}, Uzawa {(j + 1):2d} compilation (adam):   {(time_dict['simulation_compiling_adam'][2 * (i * lagrange_loops + j) + 1] - time_dict['simulation_compiling_adam'][2 * (i * lagrange_loops + j)]):8.3f}",
                    file=text_file,
                )
                print(
                    f"   Load step {(i + 1):2d}, Uzawa {(j + 1):2d} training (adam):      {(time_dict['simulation_training_adam'][2 * (i * lagrange_loops + j) + 1] - time_dict['simulation_training_adam'][2 * (i * lagrange_loops + j)]):8.3f}",
                    file=text_file,
                )
                if lbfgs_iterations > 0:
                    print(
                        f"   Load step {(i + 1):2d}, Uzawa {(j + 1):2d} compilation (L-BFGS): {(time_dict['simulation_compiling_lbfgs'][2 * (i * lagrange_loops + j) + 1] - time_dict['simulation_compiling_lbfgs'][2 * (i * lagrange_loops + j)]):8.3f}",
                        file=text_file,
                    )
                    print(
                        f"   Load step {(i + 1):2d}, Uzawa {(j + 1):2d} training (L-BFGS):    {(time_dict['simulation_training_lbfgs'][2 * (i * lagrange_loops + j) + 1] - time_dict['simulation_training_lbfgs'][2 * (i * lagrange_loops + j)]):8.3f}",
                        file=text_file,
                    )
                print(
                    f"   Load step {(i + 1):2d}, Uzawa {(j + 1):2d} prediction:           {(time_dict['simulation_prediction'][2 * (i * lagrange_loops + j) + 1] - time_dict['simulation_prediction'][2 * (i * lagrange_loops + j)]):8.3f}",
                    file=text_file,
                )
        print(f"==============================================", file=text_file)
    print(
