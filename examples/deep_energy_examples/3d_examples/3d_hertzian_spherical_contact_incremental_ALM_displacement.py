...
with an augmented Lagrangian formulation, and the prescribed displacement is
...
# Define the ALM and early stopping callbacks
...
for i in range(steps):
    ...
    print(
        f"The non-penetration condition is violated at {violations[i, 0]:.0f} points."
    )
    print(
        f"It is violated with a mean of {violations[i, 1]:.3e} "
        f"with a standard deviation of {violations[i, 2]:.3e}."
    )
    print(
        f"The maximal violation is {violations[i, 3]:.3e}, "
        f"the minimal is {violations[i, 4]:.3e}.\n"
    )
    ...
    if (displacement_bc <= max_displacement_bc) and (
        ...
        file_path_fem_compare = os.path.join(
            model_path,
            f"{simulation_case}_fem_compare_{int(displacement_bc * 1000):04}",
        )
        fem_reference.save(f"{file_path_fem_compare}.vtu")

    file_path = os.path.join(
        model_path, f"{simulation_case}_{int(displacement_bc * 1000):04}"
    )
