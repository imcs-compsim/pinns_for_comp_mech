"""
3D Block Torsion Nonlinear Displacement Incremental Training with Adam Optimizer

This module implements a Physics-Informed Neural Network (PINN) using the Deep Energy method
to solve a 3D nonlinear torsion problem on a hexahedral block. The model is trained incrementally
with progressively increasing rotation angles using the Adam optimizer.

The problem involves:
- A 3D block geometry with fixed displacement boundary conditions at x=0
- Incremental torsional loading applied at x=L with rotation angles increasing stepwise
- Hyperelastic neo-Hookean material model
- Predictions of displacement fields and stress tensors (Cauchy and first Piola-Kirchhoff)
- VTK output for visualization of results

Key components:
- Deep Energy formulation based on strain energy minimization
- Gaussian quadrature for accurate numerical integration
- Incremental training over multiple epochs with periodic model checkpoints
- Post-processing and stress computation via automatic differentiation
