"""
3D Block Under Compression - Physics-Informed Neural Network Example

This module demonstrates the application of Deep Energy Physics-Informed Neural Networks (PINNs)
to solve a 3D linear elasticity problem of a block under compressive loading. The solution uses
a variational approach based on the principle of minimum potential energy.

The problem setup includes:
- A 3D hexahedral block domain with configurable dimensions and mesh resolution
- Compression load applied on the top surface via pressure boundary condition
- Fixed displacement boundary conditions on three adjacent surfaces (x=0, y=0, z=0)
- Output includes predicted displacements and stress components (normal and shear stresses)
- Results are exported to VTK format for visualization

Key components:
- Domain discretization via Gmsh with Gauss quadrature rules for integration
- Deep feedforward neural network with output transformation enforcing boundary conditions
- Two-stage training: Adam optimizer followed by L-BFGS optimization
- Stress computation from displacement predictions using elasticity theory
