"""
3D Block Buckling Under Incremental Force Loading
==================================================

This example solves a three-dimensional block buckling problem under
incrementally applied compressive force with the Energy-based PINN (EPINN).

It is modelled using a Neo-Hookean hyperelastic material. The left side is
constrained through the output transform, while the compressive force is applied
on the right boundary through the potential energy. The load is increased in
steps, with an optional relaxation step using Adam followed by L-BFGS.
...
def potential_energy(
    ...
    external_work = (
        global_weights_boundary_t[cond_ext]
        * external_force_density
        * jacobian_boundary_t[cond_ext]
    )
...
for i in range(steps):
    ...
    file_path = os.path.join(
        model_path, f"{simulation_case}_{int(abs(force_buckling * 1e3)):02}"
    )
