...
height = 0.5
width = 1
seed_l = 101 # number of nodes along x
seed_h = 6 # number of nodes along y
...
def potential_energy(X, 
    ...
    cond_ext = boundary_selection_tag["boundary_right"]   
    u_x = outputs[:,0:1][beg_boundary:][cond_ext]
    external_force_density = force_buckling*u_x
    external_work = global_weights_boundary_t[cond_ext]*(external_force_density)*jacobian_boundary_t[cond_ext]
...
def output_transform(x, y):
    ...
    u_out = x_loc * u
    v_out = x_loc * (x_loc - length) * v
    w_out = x_loc * (x_loc - length) * w
...
# Model parameters
steps = 10
max_force_buckling = -1E-2
model_path = str(Path(__file__).parent)
simulation_case = f"3d_block_buckling_force_incremental"
...
for i in range(steps):
    ...
    file_path = os.path.join(model_path, f"{simulation_case}_{int(abs(force_buckling*1E3)):02}")
