"""
3D Contact Ring Instability Simulation on a Quarter Torus Geometry.

This example demonstrates the use of energy-based Physics-Informed Neural Networks (PINNs)
to simulate contact mechanics in a hyperelastic quarter torus under displacement loading. The simulation
models a ring instability scenario where contact occurs between the inner surface and a rigid plane.

Key components include:
- Geometry definition using Gmsh for a 3D quarter torus with specified major and tube radii.
...
- Post-processing and visualization of displacements and Cauchy stresses using PyVista.
...
quarter_circle_with_hole = QuarterTorus3D(
    center=center, major_radius=major_radius, tube_radius=tube_radius, mesh_size=0.015
)
...
def on_contact(x):
    """Compute on contact for this example setup.

    Args:
        x: Input coordinates used to evaluate the function.

    Returns:
        bool: Result of the `on_contact` evaluation.
    """
    return np.isclose(
        (np.sqrt(x[0] ** 2 + x[1] ** 2) - major_radius) ** 2 + x[2] ** 2, tube_radius**2
    )
...
boundary_selection_map = [
    {"boundary_function": on_contact, "tag": "on_contact"},
...
grid.save("3d_contact_ring_instability_quarter_torus.vtu")
