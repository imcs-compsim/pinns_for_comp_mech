"""
3D Block Torsion Analysis with Nonlinear Displacement Incremental Training using PyTorch.

This module implements a Physics-Informed Neural Network (PINN) using the Deep Energy method
to solve a 3D nonlinear torsion problem. The model is trained incrementally with increasing
rotation angles, predicting displacement fields and computing Cauchy and first Piola-Kirchhoff
stress tensors for a hyperelastic material (Neo-Hookean).

Key features:
- 3D hexahedral mesh generation using Gmsh
- Variational formulation based on strain energy minimization
- Incremental loading with Adam optimizer
- Output transformation applying boundary-prescribed rotations
- VTK export of predictions for visualization
