...
A nonlinear Neo-Hookean material model is used.
...
def compareModelPredictionAndAnalyticalSolution(model):
    ...
    sigma_rr, sigma_rtheta, sigma_theta_r, sigma_theta = (
        polar_transformation_2d_tensor_new(sigma_xx, sigma_yy, sigma_xy, sigma_xy, r_x)
    )
...
sigma_rr, sigma_rtheta, sigma_theta_r, sigma_theta = polar_transformation_2d_tensor_new(
    sigma_xx, sigma_yy, sigma_xy, sigma_xy, X
)
p_rr, p_rtheta, p_theta_r, p_theta = polar_transformation_2d_tensor_new(
    p_xx, p_yy, p_xy, p_yx, X
)
...
combined_stress_polar = tuple(
    np.vstack(
        (
            np.array(sigma_rr.flatten().tolist()),
            np.array(sigma_theta.flatten().tolist()),
            np.array(sigma_rtheta.flatten().tolist()),
        )
    )
)
combined_first_piola_stress = tuple(
    np.vstack(
        (
            np.array(p_xx.flatten().tolist()),
            np.array(p_yy.flatten().tolist()),
            np.array(p_xy.flatten().tolist()),
        )
    )
)
combined_p_polar = tuple(
    np.vstack(
        (
            np.array(p_rr.flatten().tolist()),
            np.array(p_theta.flatten().tolist()),
            np.array(p_rtheta.flatten().tolist()),
        )
    )
)

file_path = os.path.join(os.getcwd(), "Lame_problem_quarter_nonlinear")
