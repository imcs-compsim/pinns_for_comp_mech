...
def points_at_top(on_boundary, x):
    ...
    """
    cond_points_top = np.isclose(x[1], 0)
    return on_boundary and cond_points_top
...
def output_transform(x, y):
    ...
    return bkd.concat(
        [
            ...
            (v * (-y_loc) + applied_disp_y * youngs_modulus) / youngs_modulus,
        ],
        axis=1,
    )
...
def polar_transformation_2d_tensor(T_xx, T_yy, T_xy, T_yx, X):
    ...
    return (
        T_rr.astype(np.float32),
        T_rtheta.astype(np.float32),
        T_thetar.astype(np.float32),
        T_thetatheta.astype(np.float32),
    )
...
sigma_rr_pred, sigma_rtheta_pred, sigma_theta_r_pred, sigma_theta_pred = (
    polar_transformation_2d_tensor(
        sigma_xx_pred, sigma_yy_pred, sigma_xy_pred, sigma_yx_pred, X
    )
)
...
combined_stress_polar_pred = tuple(
    np.vstack(
        (
            np.array(sigma_rr_pred.flatten().tolist()),
            np.array(sigma_theta_pred.flatten().tolist()),
            np.array(sigma_rtheta_pred.flatten().tolist()),
        )
    )
)
...
sigma_rr_fem, sigma_rtheta_fem, sigma_theta_r_fem, sigma_theta_fem = (
    polar_transformation_2d_tensor(
        sigma_xx_fem, sigma_yy_fem, sigma_xy_fem, sigma_xy_fem, X
    )
)
...
combined_stress_polar_fem = tuple(
    np.vstack(
        (
            np.array(sigma_rr_fem.flatten().tolist()),
            np.array(sigma_theta_fem.flatten().tolist()),
            np.array(sigma_rtheta_fem.flatten().tolist()),
        )
    )
)
...
file_path = os.path.join(
    os.getcwd(), "deep_energy_hertzian_normal_contact_nonlinear_displacement"
)
