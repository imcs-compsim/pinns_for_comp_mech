...
class QuarterDisc(object):
    ...
    def generateGmshModel(self, visualize_mesh=False):
        ...
        p1 = gmsh_model.geo.addPoint(self.center[0] - self.radius, self.center[1], 0, lcar)             # left
        p2 = gmsh_model.geo.addPoint(self.center[0], self.center[1] - self.radius, 0, lcar)             # bottom
        p5 = gmsh_model.geo.addPoint(self.center[0], self.center[1] - 0.8, 0, lcar)                      # right base of square
...
Quarter_Disc = QuarterDisc(radius=radius, center=center, mesh_size=0.04, angle=255, refine_times=20, gmsh_options=gmsh_options)
...
quad_rule = GaussQuadratureRule(rule_name="gauss_legendre", element_type="simplex", dimension=2, ngp=3) # gauss_legendre gauss_labotto
