...
def potential_energy(
    ...
    gap_n = tf.math.divide_no_nan(
        gap_y, tf.math.abs(mapped_normal_boundary_t[:, 1:2][cond])
    )
    eta = 1e3  # 1e3
...
def get_stress_XY(x, y, X):
    """Compute get stress X Y for this example setup.
    ...
    Returns:
        Any: Computed value returned by `get_stress_XY`.
    """
...
def get_stress_YY(x, y, X):
    """Compute get stress Y Y for this example setup.
    ...
    Returns:
        Any: Computed value returned by `get_stress_YY`.
    """
    eps_xx, eps_yy, eps_xy = elastic_strain_2d(x, y)
...
def get_stress_XX(x, y, X):
    """Compute get stress X X for this example setup.
    ...
    Returns:
        Any: Computed value returned by `get_stress_XX`.
    """
    eps_xx, eps_yy, eps_xy = elastic_strain_2d(x, y)
    nu, lame, shear, youngs_modulus = problem_parameters()
...
if add_external_data:
    ...
    observe_sigma_xx = dde.PointSetOperatorBC(
        ex_data_xy, ex_data_stress[:, 0:1], get_stress_XX
    )
    observe_sigma_yy = dde.PointSetOperatorBC(
        ex_data_xy, ex_data_stress[:, 1:2], get_stress_YY
    )
    observe_sigma_xy = dde.PointSetOperatorBC(
        ex_data_xy, ex_data_stress[:, 2:3], get_stress_XY
    )
