"""
Deep Energy PINN for single block compression-traction with localized loading.

This module demonstrates a Physics-Informed Neural Network (PINN) using the Deep Energy
method for 2D elastic block analysis under localized traction on the right boundary.
It trains a neural network to satisfy energy minimization principles and outputs predictions
in VTK format.

Key features:
- 2D block geometry with Gmsh integration
- Gaussian quadrature integration for domain and boundary integrals
- Deep Energy formulation based on potential energy (strain energy - external work)
- Localized pressure loading on a portion of the right boundary
- Neural network output transformation with geometric scaling
- Model training with Adam and L-BFGS optimizers
- VTK export of displacement and stress fields
...
def on_right(x):
    """Compute on right for this example setup.

    Args:
        x: Input coordinates used to evaluate the function.

    Returns:
        bool: Result of the `on_right` evaluation.
    """
...
file_path = os.path.join(
    os.getcwd(), "deep_energy_single_block_compression_traction_paper_localized"
)
