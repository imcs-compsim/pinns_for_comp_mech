"""
Single block compression example using Deep Energy Method with Physics-Informed Neural Networks.

This module demonstrates the application of Deep Energy PINNs to solve a 2D plane strain
compression problem on a unit square block. The problem is solved using a variational
(energy-based) approach where the neural network minimizes the total potential energy
(strain energy minus external work).

The script:
1. Sets up a 2D block geometry with Gmsh and defines Gauss quadrature rules
2. Defines the potential energy functional for plane strain elasticity
3. Trains a neural network with boundary conditions (applied displacement at top)
4. Compares predicted displacements and stresses against analytical solutions
5. Exports results to VTK format with error fields
