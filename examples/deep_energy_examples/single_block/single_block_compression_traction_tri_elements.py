"""
Deep Energy Method for 2D Block Compression/Traction with Triangular Elements

This module implements a Physics-Informed Neural Network (PINN) using the Deep Energy method
to solve a 2D linear elasticity problem for a square block under compression/traction loading.
The problem uses triangular finite elements and Gauss quadrature for numerical integration.

The script:
- Generates a 2D block geometry with Gmsh
- Sets up quadrature rules for domain and boundary integrals
- Defines a potential energy functional for the deep energy formulation
- Trains a neural network to minimize the total energy (internal + external)
- Validates predictions against analytical solutions
- Exports displacement and stress fields to VTK format with error metrics
