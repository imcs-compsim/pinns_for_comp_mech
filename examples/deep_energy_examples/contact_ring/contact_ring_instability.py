...
quarter_circle_with_hole = RingQuarter(center=[0,0,0], inner_radius=0.9, outer_radius=1, mesh_size=0.025, gmsh_options=gmsh_options)
...
def output_transform(x, y):
    ...
    return bkd.concat([u*(x_loc*y_loc)/e_modul, (v*(-y_loc)+applied_disp_y*e_modul)/e_modul], axis=1)
...
file_path = os.path.join(os.getcwd(), f"deep_energy_contact_ring_instability_p_{abs(applied_disp_y)}")
