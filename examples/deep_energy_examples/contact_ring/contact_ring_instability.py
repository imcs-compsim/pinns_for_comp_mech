...
to simulate contact mechanics in a hyperelastic quarter circle arc under displacement loading. The
simulation models a ring instability scenario where contact occurs between the inner
surface and a rigid plane.

Key components include:
- Geometry definition using Gmsh for a 2D quarter circle arc with specified major and minor radii.
...
from compsim_pinns.geometry.gmsh_models import RingQuarter
...
quarter_circle_with_hole = RingQuarter(
    center=[0, 0, 0],
    inner_radius=0.9,
    outer_radius=1,
    mesh_size=0.025,
...
file_path = os.path.join(os.getcwd(), f"contact_ring_instability_{abs(applied_disp_y)}")
