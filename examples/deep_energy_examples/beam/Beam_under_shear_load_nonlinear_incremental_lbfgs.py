"""
Physics-Informed Neural Network (PINN) for nonlinear beam analysis under incremental shear loading.

This module implements a deep energy method using physics-informed neural networks to simulate
a 2D beam undergoing nonlinear deformation under incremental shear loads. The solution uses
Neo-Hookean hyperelasticity with plane strain conditions and employs an incremental loading
strategy with stabilization phases.

The workflow includes:
1. Mesh generation via Gmsh for a 2D rectangular domain
2. Training with stabilization phase followed by incremental load application
3. Post-processing with stress computation and VTU file export for visualization
...
def potential_energy(
    ...
    if model.data.current_epoch is not None:
        ...
        if not apply_load:
            ...
            shear_load_chunk = shear_load

    external_force_density = -shear_load_chunk * u_y
    external_work = (
...
file_path = os.path.join(os.getcwd(), "Beam_under_shear_load_nonlinear_incremental")
...
file_path = os.path.join(os.getcwd(), "Beam_under_shear_load_nonlinear_single")
# epoch_tracker = dde.callbacks.EpochTracker(period=10)
model_saver = SaveModelVTU(
    op=cauchy_stress_2D,
    period=500,
    stabilization_epoch=(epochs + stabilization_model_epoch),
    filename=file_path,
)
