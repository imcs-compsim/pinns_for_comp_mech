"""
Nonlinear Beam Under Incremental Shear Loading in a mixed formulation
==================================================
...
A mixed formulation predicts displacement and first Piola stress components.
...
from compsim_pinns.hyperelasticity.hyperelasticity_utils import (
    cauchy_stress_2D_mixed_formulation,
...
def potential_energy(
    ...
    # Mixed-formulation consistency and stress-equilibrium terms
    firstpiola_xx_u, firstpiola_yy_u, firstpiola_xy_u, firstpiola_yx_u = (
        first_piola_stress_tensor_2D(inputs, outputs[:, 0:2])
    )
    residuum_firstpiolafirstpiola = (
        (firstpiola_xx_u - outputs[:, 2:3]) ** 2
        + (firstpiola_xy_u - outputs[:, 3:4]) ** 2
        + (firstpiola_yx_u - outputs[:, 4:5]) ** 2
        + (firstpiola_yy_u - outputs[:, 5:6]) ** 2
    )
    consistency_domain = (
        global_element_weights_t[:, 0:1]
        * global_element_weights_t[:, 1:2]
        * (residuum_firstpiolafirstpiola[beg_pde:beg_boundary])
        * jacobian_t
    )
    consistency_boundary = (
        global_weights_boundary_t
        * (residuum_firstpiolafirstpiola[beg_boundary:])
        * jacobian_boundary_t
    )
    ...
    divergence = (
        dde.grad.jacobian(outputs, inputs, i=2, j=0)
        + dde.grad.jacobian(outputs, inputs, i=3, j=1)
    ) ** 2 + (
        dde.grad.jacobian(outputs, inputs, i=4, j=0)
        + dde.grad.jacobian(outputs, inputs, i=5, j=1)
    ) ** 2
...
# Define the feature and output transforms
...
def output_transform(x, y):
    ...
    return bkd.concat(
        [
            x_loc / l_beam * h_beam * u,
            x_loc / l_beam * h_beam * v,
            (l_beam - x_loc) / l_beam * youngs_modulus * P_xx,
            2
            / h_beam
            * (h_beam / 2 - y_loc)
            * 2
            ...
            * youngs_modulus
            * P_xy,
            -shear_load + (l_beam - x_loc) / l_beam * youngs_modulus * P_yx,
            2
            / h_beam
            * (h_beam / 2 - y_loc)
            * 2
...
if earlystopping:
    if earlystopping_choice == "loss":
        ...
        early = WeightsBiasPlateauStopping(
            patience=500, min_delta=1e-5, norm_choice="fro"
        )
    else:
...
for i in range(steps):
    ...
    sigma_xx, sigma_yy, sigma_xy, _ = model.predict(
        points, operator=cauchy_stress_2D_mixed_formulation
    )
    ...
    if int(round(shear_load * 1e3)) in set(range(1, 11)):
        ...
        fem_reference = pv.read(
            fem_path
            + f"/fem_reference/fem_reference_2d_bending_beam_{int(round(shear_load * 1e3)):02}.vtu"
        )
        ...
        (
            ...
            points_fem[:, 0:2], operator=cauchy_stress_2D_mixed_formulation
        )
...
# Save the relative L2 errors for displacement and stress
np.savez(
