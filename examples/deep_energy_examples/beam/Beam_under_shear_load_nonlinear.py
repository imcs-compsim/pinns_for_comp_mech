...
This module demonstrates the Deep Energy approach to solve a nonlinear hyperelastic beam problem
under shear loading. It uses a neural network trained via energy minimization to predict displacement
fields and compute derived quantities (stress, strain, deformation gradient).

The workflow includes:
- 2D beam geometry generation using Gmsh
- Neo-Hookean hyperelastic material model with configurable elastic properties
- Deep Energy formulation with Gauss quadrature integration
- Model training with Adam and L-BFGS optimizers
- Post-processing and comparison with FEM reference results via PyVista

Key features:
    - Enforces displacement boundary conditions through output transformation
    - Computes internal strain energy and external work
    - Saves and restores trained models for inference
    - Exports results to VTU format with pointwise error metrics
...
model_path = (
    ...
    + f"/pretrained_models/deep_energy_examples/beam/beam_nonlinear"
)
...
file_path = (
    ...
    + f"/reference_results/beam/bending_beam_test-structure.pvd"
)
...
deformation_gradient = np.column_stack(
    (f_xx, f_yy, f_yx, f_xy, np.zeros_like(f_xx), np.zeros_like(f_xx))
)
# first_piola_stress = np.column_stack((p_xx, p_yy, p_xy))
displacement = np.column_stack(
    (output[:, 0:1], output[:, 1:2], np.zeros_like(output[:, 0:1]))
)
