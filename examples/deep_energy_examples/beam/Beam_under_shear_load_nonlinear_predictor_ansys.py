...
The network is reinitialized for each load step and trained with Adam followed by L-BFGS.

Results are exported as VTU files for post-processing, while displacement
predictions are also written to Matrix Market files for external solver input.
Errors, timings, and loss information are also saved.
"""
...
displacement_to_4C_save_path = (
    f"{model_path}/Beam_under_shear_load_nonlinear_displacement_to_4C"
)
mesh_4C_input_path = (
    str(Path(__file__).parent.parent.parent.parent)
    + f"/paper-epinn/2d_bending_beam/hyperelastic_bending_beam.exo"
)
...
for i in range(steps):
    ...
    mmwrite(
        f"{displacement_to_4C_save_path}_{int(shear_load * 1e3):02}",
