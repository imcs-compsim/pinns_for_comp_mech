"""
3D Lamé Problem with Internal Pressure using Physics-Informed Neural Networks.

This module demonstrates the solution of a 3D linear elasticity problem (Lamé problem)
with internal pressure using DeepXDE's mixed formulation approach. The problem involves
a spherical shell with inner radius R_i and outer radius R_o subjected to internal pressure.

The analytical solution is compared with PINN predictions for both displacement and stress fields.
The solution uses DeepXDE's FNN architecture with output transform to enforce boundary conditions
...
Key Features:
    - 3D mixed formulation with displacement and stress outputs
    - Spherical coordinate transformations for analytical validation
    - Mixed boundary conditions (Neumann and symmetric)
    - Two-stage training: ADAM optimization followed by L-BFGS
    - Comparison of numerical and analytical solutions
    - VTK output generation for post-processing
...
def apply_pressure(x, y, X):
    ...
    """
    Tx, Ty, Tz, On, Tt_1, Tt_2 = get_tractions_mixed_3d(x, y, X)
...
bc4 = dde.OperatorBC(geom, apply_pressure, boundary_inner)
...
model_path = (
    ...
    + f"/pretrained_models/elasticity_3d/lame/lame_3d"
)
