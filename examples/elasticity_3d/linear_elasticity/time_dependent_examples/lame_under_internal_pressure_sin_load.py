"""
3D time-dependent Lame problem with sinusoidal internal pressure loading using Physics-Informed Neural Networks (PINNs).

This module solves a 3D linear elasticity problem for a thick spherical shell (Lame problem)
subjected to time-varying internal pressure with sinusoidal loading. The problem is formulated
as a mixed formulation with displacements and stresses as outputs, solved using DeepXDE with
a fully connected neural network.

Key features:
- Time-dependent spherical shell domain (inner radius=1, outer radius=2)
- Sinusoidal pressure loading: p(t) = pressure * sin(2πt)
- Mixed formulation: displacement and stress components as network outputs
- Neumann boundary conditions on outer surface (zero traction)
- Mixed boundary conditions on inner surface (pressure + zero shear)
- Initial conditions for velocity components
- Output transformations enforcing essential boundary conditions
- Comparison with analytical solution in spherical coordinates
...
from compsim_pinns.elasticity.elasticity_utils import (
    get_tractions_mixed_3d_spacetime,
    pde_mixed_3d_time,
    problem_parameters,
)
...
from compsim_pinns.postprocess.save_normals_tangentials_to_vtk import (
    export_normals_tangentials_to_vtk,
)

path_to_step_file = (
    str(Path(__file__).parent.parent.parent) + f"/step_files/lame_3d.stp"
)
...
export_normals_tangentials_to_vtk(
    ...
    file_name="lame_boundary_normals",
)
...
def boundary_outer(x, on_boundary):
    ...
    return on_boundary and np.isclose(
        np.linalg.norm(space_coords - center, axis=-1), radius_outer
    )
...
def boundary_inner(x, on_boundary):
    ...
    space_coords = x[:3]
    return on_boundary and np.isclose(
        np.linalg.norm(space_coords - center, axis=-1), radius_inner
    )  # and cond_on_edge


def apply_pressure(x, y, X):
    """Compute apply pressure for this example setup.
    ...
    Returns:
        Any: Computed value returned by `apply_pressure`.
    """
    Tx, Ty, Tz, On, Tt_1, Tt_2 = get_tractions_mixed_3d_spacetime(x, y, X)
    _, _, _, cond = calculate_boundary_normals_3D(X, spaceTimeDomain)
    t_loc = x[:, 3:4][cond]  # time only for boundary points

    return On + pressure * (bkd.sin(2 * np.pi * t_loc))


def zero_neumann_in_normal(x, y, X):
    """Compute zero neumann in normal for this example setup.
    ...
    Returns:
        Any: Computed value returned by `zero_neumann_in_normal`.
    """
    Tx, Ty, Tz, On, Tt_1, Tt_2 = get_tractions_mixed_3d_spacetime(x, y, X)
...
def zero_neumann_in_tangential1(x, y, X):
    """Compute zero neumann in tangential1 for this example setup.
    ...
    Returns:
        Any: Computed value returned by `zero_neumann_in_tangential1`.
    """
    Tx, Ty, Tz, On, Tt_1, Tt_2 = get_tractions_mixed_3d_spacetime(x, y, X)
...
def zero_neumann_in_tangential2(x, y, X):
    """Compute zero neumann in tangential2 for this example setup.
    ...
    Returns:
        Any: Computed value returned by `zero_neumann_in_tangential2`.
    """
    Tx, Ty, Tz, On, Tt_1, Tt_2 = get_tractions_mixed_3d_spacetime(x, y, X)
...
def apply_velocity_in_x(x, y, X):
    """Compute apply velocity in x for this example setup.
    ...
    Returns:
        Any: Computed value returned by `apply_velocity_in_x`.
    """
...
def apply_velocity_in_y(x, y, X):
    """Compute apply velocity in y for this example setup.
    ...
    Returns:
        Any: Computed value returned by `apply_velocity_in_y`.
    """
...
def apply_velocity_in_z(x, y, X):
    """Compute apply velocity in z for this example setup.
    ...
    Returns:
        Any: Computed value returned by `apply_velocity_in_z`.
    """
...
bc4 = dde.OperatorBC(spaceTimeDomain, apply_pressure, boundary_inner)
...
ic_velocity_in_x = dde.icbc.OperatorBC(
    spaceTimeDomain, apply_velocity_in_x, boundary_initial
)
ic_velocity_in_y = dde.icbc.OperatorBC(
    spaceTimeDomain, apply_velocity_in_y, boundary_initial
)
ic_velocity_in_z = dde.icbc.OperatorBC(
    spaceTimeDomain, apply_velocity_in_z, boundary_initial
)
...
data = dde.data.TimePDE(
    ...
    num_boundary=geom.random_boundary_points(1).shape[
        0
    ],  # spaceDomain.random_boundary_points(1).shape[0]
    num_initial=100,
    num_test=None,
)
...
def compareModelPredictionAndAnalyticalSolution(model):
    ...
    for i, current_time in enumerate(time_element):
        ...
        X_spacetime = np.hstack(
            (r_input, np.full([len(r_input), 1], current_time, dtype=config.real(np)))
        )
        ...
        print(
            f"Relative L2 error for displacement at t={current_time}: ", rel_err_l2_disp
        )
        ...
        axs[1].plot(
            r, sigma_thetatheta_p, color=color, marker="o", markersize=5, markevery=5
        )
        ...
        axs[2].plot(
            r, sigma_phiphi_p, color=color, marker="o", markersize=5, markevery=5
        )
    ...
    analytical_legend = mlines.Line2D(
        [], [], color="black", label="Analytical Solution"
    )
    predicted_legend = mlines.Line2D(
        [],
        [],
        color="black",
        marker="o",
    ...
    fig.legend(
        handles=[analytical_legend, predicted_legend],
        loc="lower center",
        ncol=2,
        fontsize=14,
