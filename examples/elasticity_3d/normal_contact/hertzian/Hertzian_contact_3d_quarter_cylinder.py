"""
3D Hertzian contact problem with quarter-cylinder geometry using mixed formulation.
Enforces contact conditions via Fischer-Burmeister nonlinear complementarity function.
...
from compsim_pinns.contact_mech.contact_utils import (
    zero_complementarity_function_based_fischer_burmeister_3d,
    zero_tangential_traction_component1_3d,
    zero_tangential_traction_component2_3d,
)
from compsim_pinns.elasticity import elasticity_utils
from compsim_pinns.elasticity.elasticity_utils import (
    apply_zero_neumann_x_mixed_formulation,
    apply_zero_neumann_y_mixed_formulation,
    apply_zero_neumann_z_mixed_formulation,
    pde_mixed_3d,
    problem_parameters,
)
...
path_to_step_file = (
    ...
    + f"/step_files/hertzian_quarter_cylinder.stp"
)

curve_info = {"7": 15, "9": 15, "14": 8, "18": 8, "8": 40, "6": 25, "2": 15, "16": 15}
...
def boundary_not_contact(x, on_boundary):
    """Check whether a point satisfies the `boundary_not_contact` boundary condition.
    ...
    Returns:
        bool: Result of the `boundary_not_contact` evaluation.
    """
    return (
        on_boundary
        and np.isclose(np.linalg.norm(x[:2] - center[:2], axis=-1), radius)
        and (x[0] < b_limit)
    )


def boundary_contact(x, on_boundary):
    """Check whether a point satisfies the `boundary_contact` boundary condition.
    ...
    Returns:
        bool: Result of the `boundary_contact` evaluation.
    """
    return (
        on_boundary
        and np.isclose(np.linalg.norm(x[:2] - center[:2], axis=-1), radius)
...
loss_weights = [
    ...
    w_zero_traction_y,
    w_zero_traction_z,
    w_zero_tangential_traction_component1,
    w_zero_tangential_traction_component2,
    w_zero_fischer_burmeister,
]

model_path = (
    str(Path(__file__).parent.parent.parent.parent)
    + f"/pretrained_models/elasticity_3d/hertzian/hertzian_3d"
)
restore_model = False
...
solutionFieldOnMeshToVtk3D(
    ...
    file_name="3D_hertzian_contact_quarter_cylinder",
