...
s_x = -p_max * (
    (1 + 2 * (z**2 / b**2)) / (np.sqrt(1 + z**2 / b**2)) - 2 * np.abs(z / b)
)
...
ex_data_xyz_2 = np.hstack(
    (np.zeros_like(y_coord), y_coord, -0.5 * np.ones_like(y_coord))
)
...
model_path = (
    ...
    + f"/pretrained_models/elasticity_3d/hertzian_enhanced/hertzian_3d_enhanced_analytical"
)
restore_model = False
