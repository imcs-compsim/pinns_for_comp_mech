...
def fun_sigma_yy(x, y, X):
    """Calculate the value of sigma_yy on the boundary."""
...
def weak_form_x(
    ...
    """Compute the x part of the weak form for this example setup.
    ...
    # body forces
    f_x = lame * (
        4 * np.pi**2 * bkd.cos(2 * np.pi * x_s) * bkd.sin(np.pi * y_s)
        - np.pi * bkd.cos(np.pi * x_s) * Q_param * y_s**3
    ) + shear * (
        9 * np.pi**2 * bkd.cos(2 * np.pi * x_s) * bkd.sin(np.pi * y_s)
        - np.pi * bkd.cos(np.pi * x_s) * Q_param * y_s**3
    )
    ...
    weighted_residual_x = (
        g_weights[:, 0:1]
        * g_weights[:, 1:2]
        * (residual_x - body_force_residual_x)
        * g_jacobian
    )
...
def weak_form_y(
    ...
    """Compute the y part of the weak form for this example setup.
    ...
    # body forces
    f_y = lame * (
        -3 * bkd.sin(np.pi * x_s) * Q_param * y_s**2
        + 2 * np.pi**2 * bkd.sin(2 * np.pi * x_s) * bkd.cos(np.pi * y_s)
    ) + shear * (
        -6 * bkd.sin(np.pi * x_s) * Q_param * y_s**2
        + 2 * np.pi**2 * bkd.sin(2 * np.pi * x_s) * bkd.cos(np.pi * y_s)
        + np.pi**2 * bkd.sin(np.pi * x_s) * Q_param * y_s**4 / 4
    )
    ...
    weighted_residual_y = (
        g_weights[:, 0:1]
        * g_weights[:, 1:2]
        * (residual_y - body_force_residual_y)
        * g_jacobian
    )
...
unstructuredGridToVTK(
    ...
    offset,
    cell_types,
    pointData={"displacement": combined_disp_pred, "stress": combined_stress_pred},
)
