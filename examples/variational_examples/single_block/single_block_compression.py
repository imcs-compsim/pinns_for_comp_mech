"""
2D block under uniaxial compression using Variational Physics-Informed Neural Networks (VPINNs).
Solves the elasticity problem in plane stress formulation with weak form residuals.
...
def output_transform(x, y):
    ...
    return bkd.concat(
        [
            u * (x_loc),
            v * (y_loc),
            pressure + sigma_xx * (l_beam - x_loc),
            sigma_yy * (h_beam - y_loc),
            sigma_xy * (l_beam - x_loc) * (x_loc) * (h_beam - y_loc) * (y_loc),
        ],
        axis=1,
    )
...
A = [
    [
        np.cos(theta_radian) ** 2,
        np.sin(theta_radian) ** 2,
        2 * np.sin(theta_radian) * np.cos(theta_radian),
    ],
    [
        np.sin(theta_radian) ** 2,
        np.cos(theta_radian) ** 2,
    ...
    [
        ...
        np.sin(theta_radian) * np.cos(theta_radian),
        np.cos(theta_radian) ** 2 - np.sin(theta_radian) ** 2,
    ],
]
A = np.array(A)
...
for i in range(len(sigma_rr)):
    sigma_analytical[i : i + 1, :] = np.matmul(
        np.linalg.inv(A[:, :, i]), sigma_combined_radial.T[:, i : i + 1]
    ).T
...
combined_stress_analytical = tuple(
    np.vstack(
        (
            np.array(sigma_analytical[:, 0].flatten().tolist()),
            np.array(sigma_analytical[:, 1].flatten().tolist()),
            np.array(sigma_analytical[:, 2].flatten().tolist()),
        )
    )
)
combined_disp_analytical = tuple(
    np.vstack(
        (
            np.array(u_combined[:, 0].flatten().tolist()),
            np.array(u_combined[:, 1].flatten().tolist()),
            np.zeros(u_combined[:, 1].shape[0]),
        )
    )
)
