...
from compsim_pinns.elasticity.elasticity_utils import (
    ...
    lin_iso_elasticity_plane_stress,
...
elasticity_utils.lame = (
    e_predicted * nu_predicted / ((1 + nu_predicted) * (1 - 2 * nu_predicted))
)
...
inv_data = (
    ...
    + "/elasticity_2d/data/lame/Inverse_mixed_variable"
)
data = np.loadtxt(inv_data)
np.random.seed(42)
np.random.shuffle(data)
data = data[:100]
data_xy, data_u, _, _, data_sigma_yy = (
    data[:, 0:2],
    data[:, 2:3],
    data[:, 3:4],
    data[:, 4:5],
...
def output_transform(x, y):
    ...
    return bkd.concat(
        [u * (x_loc), v * (y_loc), sigma_xx, sigma_yy, sigma_xy * x_loc * y_loc], axis=1
    )
...
variable = dde.callbacks.VariableValue(
    external_var_list, period=10, filename=parameter_file_name, precision=4
)
...
model.compile(
    ...
    external_trainable_variables=external_var_list,
)
losshistory, train_state = model.train(
    epochs=n_iter_adam, callbacks=[variable], display_every=100
)

model.compile(
    "L-BFGS-B", loss=mean_squared_error, external_trainable_variables=external_var_list
)
...
df_sliced = df[
    :350
]  # pickupt only epochs until 3500. Since we have period of 10, the last value will be 350
...
ax.vlines(
    x=n_iter_adam,
    ymin=df_sliced[2].min() * 1.5,
    ymax=df_sliced[1].max() * 1.1,
    linestyles="--",
    colors="k",
)
ax.hlines(
    y=e_actual,
    xmin=0,
    xmax=df_sliced[0].max(),
    linestyles="-.",
    colors="b",
    label=f"Actual $E={e_actual}$",
)
ax.hlines(
    y=nu_actual,
    xmin=0,
...
ax.hlines(
    y=pressure_inlet_actual,
    xmin=0,
    xmax=df_sliced[0].max(),
    linestyles="-.",
    colors="g",
    label=r"Actual $P=$" + f"{pressure_inlet_actual}",
)

ax.annotate(
    r"ADAM $\Leftarrow$ ", xy=[n_iter_adam - 600, df_sliced[1].max() * 1.05], size=25
)
ax.annotate(
    r"$\Rightarrow$ L-BGFS", xy=[n_iter_adam + 50, df_sliced[1].max() * 1.05], size=25
)
