...
from compsim_pinns.elasticity.elasticity_utils import (
    ...
    lin_iso_elasticity_plane_strain,
    problem_parameters,
    zero_neumann_x_mixed_formulation,
    zero_neumann_y_mixed_formulation,
)
from compsim_pinns.geometry.custom_geometry import GmshGeometry2D, GmshGeometryElement
...
from compsim_pinns.vpinns.quad_rule import (
    GaussQuadratureRule,
    get_test_function_properties,
)
...
n_test_func = 5
test_function, test_function_derivative = get_test_function_properties(
    n_test_func, coord_quadrature, approach="2"
)
...
def weak_form_1(
    ...
    g_weights,
    g_test_function,
    g_test_function_derivative,
):
    """Compute weak form 1 for this example setup.
    ...
    Args:
        ...
        n_e: Value for n e.
        n_gp: Value for n gp.
        g_jacobian: Value for g jacobian.
        g_weights: Value for g weights.
        g_test_function: Value for g test function.
        g_test_function_derivative: Value for g test function derivative.

    Returns:
        Any: Computed value returned by `weak_form_1`.
    """
    ...
    weighted_residual_x = (
        g_weights[:, 0:1] * g_weights[:, 1:2] * (residual) * g_jacobian
    )
...
def weak_form_2(
    ...
    """Compute weak form 2 for this example setup.
    ...
    Returns:
        Any: Computed value returned by `weak_form_2`.
    """
    vx = g_test_function[:, 0:1]
    vy = g_test_function[:, 1:2]
...
def weak_form_3(
    ...
    """Compute weak form 3 for this example setup.
    ...
    Returns:
        Any: Computed value returned by `weak_form_3`.
    """
    vx = g_test_function[:, 0:1]
    vy = g_test_function[:, 1:2]
...
def weak_form_4(
    ...
    """Compute weak form 4 for this example setup.
    ...
    Returns:
        Any: Computed value returned by `weak_form_4`.
    """
    vx = g_test_function[:, 0:1]
    vy = g_test_function[:, 1:2]
...
def weak_form_5(
    ...
    """Compute weak form 5 for this example setup.
    ...
    Returns:
        Any: Computed value returned by `weak_form_5`.
    """
    vx = g_test_function[:, 0:1]
    vy = g_test_function[:, 1:2]
...
def weak_form_6(
    ...
    """Compute weak form 6 for this example setup.
    ...
    Returns:
        Any: Computed value returned by `weak_form_6`.
    """
    vx = g_test_function[:, 0:1]
    vy = g_test_function[:, 1:2]
...
def mean_squared_error(y_true, y_pred):
    """Compute mean squared error for this example setup.

    Args:
        y_true: Value for y true.
        y_pred: Value for y pred.

    Returns:
        Any: Computed value returned by `mean_squared_error`.
    """
...
def calculate_loss():
    """Compute the `calculate_loss` loss contribution.

    Returns:
        Any: Computed value returned by `calculate_loss`.
    """
    losses = np.hstack(
        (
...
def compareModelPredictionAndAnalyticalSolution(model):
    ...
    rel_err_l2_disp = np.linalg.norm(
        u_rad.flatten() - u_rad_pred.flatten()
    ) / np.linalg.norm(u_rad)
    print("Relative L2 error for displacement: ", rel_err_l2_disp)

    sigma = np.vstack(
        (sigma_rr_analytical.reshape(-1, 1), sigma_theta_analytical.reshape(-1, 1))
    )
    ...
    axs[0].plot(
        ...
        label=r"Analytical $\sigma_{rr}/R_i$",
        color="tab:blue",
    )
    axs[0].plot(
        r / radius_inner,
        sigma_rr / radius_inner,
        label=r"Predicted $\sigma_{rr}/R_i$",
        color="tab:blue",
        marker="o",
        markersize=5,
        markevery=5,
    ...
    axs[0].plot(
        ...
        label=r"Analytical $\sigma_{\theta\theta}/R_i$",
        color="tab:orange",
    )
    axs[0].plot(
        r / radius_inner,
        sigma_theta / radius_inner,
        label=r"Predicted $\sigma_{\theta\theta}/R_i$",
        color="tab:orange",
        marker="o",
        markersize=5,
        markevery=5,
    ...
    axs[1].plot(
        r / radius_inner,
        u_rad / radius_inner,
        label=r"Analytical $u_r/R_i$",
        color="tab:orange",
    )
    axs[1].plot(
        r / radius_inner,
        u_rad_pred / radius_inner,
        label=r"Predicted $u_r/R_i$",
        color="tab:orange",
        marker="o",
        markersize=5,
        markevery=5,
...
def computeError(model):
    ...
    sigma_xx, sigma_yy, sigma_xy = output[:, 2:3], output[:, 3:4], output[:, 4:5]
    sigma_rr, sigma_theta, sigma_rtheta = polar_transformation_2d(
        sigma_xx, sigma_yy, sigma_xy, pts_xy
    )
...
unstructuredGridToVTK(
    ...
    cell_types,
    pointData={
        "displacement": combined_disp_pred,
        "stress": combined_stress_pred,
        "stress_polar": combined_stress_polar_pred,
    },
)
