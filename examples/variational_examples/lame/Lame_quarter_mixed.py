...
def weak_form_x(
    ...
    """Compute weak form x for this example setup.
    ...
    Returns:
        Any: Computed value returned by `weak_form_x`.
    """
    if residual_form == "1":
    ...
    weighted_residual_x = (
        g_weights[:, 0:1] * g_weights[:, 1:2] * (residual_x) * g_jacobian
    )
...
def weak_form_y(
    ...
    """Compute weak form y for this example setup.
    ...
    Returns:
        Any: Computed value returned by `weak_form_y`.
    """
    if residual_form == "1":
        vx = g_test_function[:, 0:1]
        vy = g_test_function[:, 1:2]
    ...
    weighted_residual_y = (
        g_weights[:, 0:1] * g_weights[:, 1:2] * (residual_y) * g_jacobian
    )
