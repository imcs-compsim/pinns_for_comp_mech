...
from compsim_pinns.geometry.gmsh_models import Line_1D
...
def func(x):
    ...
    gtemp = -0.1 * (omega**2) * np.sin(omega * x) - (2 * r1**2) * (np.tanh(r1 * x)) / (
        (np.cosh(r1 * x)) ** 2
    )
    return -amp * gtemp
...
def cosh(x):
    """Compute cosine hyperbolic function for different backends"""
...
quad_rule = GaussQuadratureRule(
    rule_name="gauss_lobatto", dimension=1, ngp=40
)  # gauss_legendre
