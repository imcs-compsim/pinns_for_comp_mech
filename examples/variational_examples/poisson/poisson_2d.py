"""
This example demonstrates the application of variational physics-informed neural networks (VPINNs)
to solve a 2D Poisson equation. The code defines the geometry of the problem using Gmsh,
sets up the weak form of the governing equation, and trains a neural network to minimize the
residual of the weak form. The results are then compared to the exact solution, and the predictions,
exact solution, and errors are saved in VTK format for visualization.
"""
...
def u_exact(x):
    """Compute u exact for this example setup.

    Args:
        x: Input coordinates used to evaluate the function.

    Returns:
        Any: Computed value returned by `u_exact`.
    """
    x_coord = x[:, 0:1]
    y_coord = x[:, 1:2]
...
def pde(x, y):
    """Compute the residual of `pde` for the governing equation.
    ...
    Returns:
        bool: Result of the `pde` evaluation.
    """
...
def weak_form(
    ...
    """Compute weak form for this example setup.
    ...
    Returns:
        Any: Computed value returned by `weak_form`.
    """
...
def boundary(x, on_boundary):
    """Check whether a point satisfies the `boundary` boundary condition.
    ...
    Returns:
        bool: Result of the `boundary` evaluation.
    """
    return on_boundary
...
if weak:
    data = VariationalPDE(
        geom, weak_form, bc, num_domain=n_dummy, num_boundary=n_dummy, solution=u_exact
    )
else:
    data = dde.data.PDE(
        geom,
...
combined_disp_pred = tuple(
    np.vstack((u_pred.flatten(), np.zeros(u_pred.shape[0]), np.zeros(u_pred.shape[0])))
)
combined_disp_act = tuple(
    np.vstack((u_act.flatten(), np.zeros(u_act.shape[0]), np.zeros(u_act.shape[0])))
)
combined_error = tuple(
    np.vstack((error.flatten(), np.zeros(error.shape[0]), np.zeros(error.shape[0])))
)
