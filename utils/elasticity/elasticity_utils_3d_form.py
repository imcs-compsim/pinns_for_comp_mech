...
def deformation_gradient(x, y):
    ...
    dx_dXY = dde.grad.jacobian(y, x, i=0) # (batchsize, 2)
    dy_dXY = dde.grad.jacobian(y, x, i=1) # (batchsize, 2)
    
    F = bkd.stack([dx_dXY, dy_dXY], axis=1) # (batchsize, 2, 2)
...
def identity_like(tensor):
    ...
    dim = tensor.shape[1]
    identity_tensor = bkd.broadcast_to(bkd.eye(dim), [batch_size, dim, dim])
    
    return identity_tensor

def transpose(tensor):
    
    return bkd.transpose(tensor, perm=[0, 2, 1])

def determinant(A):
    
    return bkd.linalg.det(A)

def inverse(A):
    
    return bkd.linalg.inv(A)
...
def green_lagrange_strain(F, C):
    
    identity_tensor = identity_like(F)
    
    E = 0.5 * (C - identity_tensor)
    
    return E

def second_piola_stress_tensor(F, C):
    ...
    S = lame/2 * (det_F**2 - 1) * inv_C + shear * (1 - inv_C)
        
    return S

def first_piola_stress_tensor(F, S):
    
    P = bkd.matmul(F, S)
    
    return P

def cauchy_stress(F, P):
    
    det_F = determinant(F)
    transposed_F = transpose(F)
    
    T = 1/det_F * bkd.matmul(P, transposed_F)
...
#     '''
#     Calculates the momentum equation using predicted 1st Piola stresses and generates the terms for the mixed-variable formulation.

#     Parameters
#     ----------
#     x : tensor
#         The input arguments (spatial coordinates).
#     y : tensor
#         The network output.
...
#         Difference between predicted and calculated stresses in X, Y, XY, and YX directions.
#     '''
#     # Governing equation
#     P
#     p_xx_xy = dde.grad.jacobian(P, x, i=0)  # ∂P_xx/∂x
#     p_yx_yy = dde.grad.jacobian(P, x, i=1)  # ∂P_yx/∂y

#     momentum_x = sum(p_xx_xy)
#     momentum_y = sum(p_yx_yy)
...
def iso_elasticity_P(x, y):
    '''
    Calculates the difference between predicted and calculated stresses based on isotropic material law.
    ...
    Returns
    -------
    term_x, term_y, term_xy, term_yx : tensor
        Differences between predicted stresses and calculated stresses.
    '''
    
    # Extract components of P
    F = deformation_gradient(x, y)
    C = right_cauchy_green(F)
    S = second_piola_stress_tensor(F, C)
    P = first_piola_stress_tensor(F, S)
    p_xx, p_yy, p_xy, p_yx = P[:, 0, 0], P[:, 1, 1], P[:, 0, 1], P[:, 1, 0]
...
def green_lagrange_strain_tensor_1(x, y):
    ...
    f_xx, f_yy, f_xy, f_yx = F[:, 0, 0], F[:, 1, 1], F[:, 0, 1], F[:, 1, 0]
    
    # f_xx, f_yy, f_xy, f_yx = deformation_gradient(x, y)
    
    e_xx = 0.5*(f_xx * f_xx + f_yx * f_yx - 1)
...
#         elif green_lagrange_tensor_type == "second":
            
#             e_xx, e_yy, e_xy, e_yx = green_lagrange_strain_tensor_2(x, y)
            
#         else:
             
#             E = green_lagrange_strain(F, C)
...
def cauchy_stress(x, y):    #for disp based and mixed T
    ...
    inv_det_F = 1/(determinant(F))
    
    T_xx = inv_det_F * (p_xx * f_xx + p_xy * f_xy)
    T_xy = inv_det_F * (p_xx * f_yx + p_xy * f_yy)
    T_yx = inv_det_F * (p_yx * f_xx + p_yy * f_xy)
    T_yy = inv_det_F * (p_yx * f_yx + p_yy * f_yy)
...
def cauchy_stress_mixed_P(x, y):
    ...
    f_xx, f_yy, f_xy, f_yx = F[:, 0, 0], F[:, 1, 1], F[:, 0, 1], F[:, 1, 0]
    p_xx, p_yy, p_xy, p_yx = y[:, 2], y[:, 3], y[:, 4], y[:, 5]
    ...
    # Stack outputs along the last axis, expected shape: (batch_size, 4)
    return bkd.stack([T_xx, T_yy, T_xy], axis=-1)
...
def cauchy_stress_to_traction_2d(x, y, T_xx, T_yy, T_xy, normals, cond):
    ...
    print("f_yx shape:", bkd.shape(f_yx))
    print("f_yy shape:", bkd.shape(f_yx))
    print("T_xx_ shape:", bkd.shape(T_xx_))
    print("T_xy_ shape:", bkd.shape(T_xy_))
