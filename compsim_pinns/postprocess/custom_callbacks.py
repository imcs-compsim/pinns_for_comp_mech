...
class EpochTracker(Callback):
    ...
    def __init__(self, period=10):
        """Initialize EpochTracker."""
        super().__init__()
        self.period = period

    def on_epoch_begin(self):
        """Set current epoch to the epoch of the training."""
...
class SaveModelVTU(Callback):
    ...
    def __init__(self, op=None, period=100, stabilization_epoch=None, filename=None):
        super().__init__()
        """Initialize SaveModelVTU.
        Set save interval to period (defaults to 100).
        """
        self.op = op
        self.period = period
    ...
    def on_epoch_end(self):
        """Save prediction of current trained state on training points."""
        ...
        if self.epochs_since_last >= self.period:
            ...
            if self.stabilization_epoch:
                current_iteration = (
                    self.model.train_state.epoch - self.stabilization_epoch
                )
            else:
                ...
                combined_disp = tuple(
                    np.vstack(
                        (
                            np.array(displacement[:, 0].tolist()),
                            np.array(displacement[:, 1].tolist()),
                            np.array(displacement[:, 2].tolist()),
                        )
                    )
                )
            ...
            unstructuredGridToVTK(
                self.filename + "_" + str(current_iteration),
                x,
                y,
                z,
...
class LossPlateauStopping(Callback):
    ...
    def __init__(
        ...
        """Initialize LossPlateauStopping.
        Set change delta (defaults to 1E-3),
        patience (defaults to 1000),
        monitored quantity (defaults to "loss_train") and
        start point for monitoring (defaults to 0).
        """
        super().__init__()
    ...
    def on_train_begin(self):
        """Set values at the beginning of the training."""
        self.wait = 0
        self.stopped_iteration = 0

    def on_epoch_end(self):
        """Check values at the end of every epoch and stop if they are fulfilled."""
    ...
    def on_train_end(self):
        """Output to the user, that the early stopping criterion was fulfilled and training stopped."""
        if self.stopped_iteration > 0:
            print(
                f"Early stopping at iteration {self.stopped_iteration} as relative change of {self.monitor} was below {self.min_delta:.1E} for at least {self.patience} iterations."
            )

    def get_monitor_value(self):
        """Get the monitored quantity."""
...
class WeightsBiasPlateauStopping(Callback):
    ...
    def __init__(
        self, min_delta=1e-2, patience=1000, norm_choice="fro", start_from_iteration=0
    ):
        """Initialize WeightsBiasPlateauStopping.
        Set change delta (defaults to 1E-2),
        patience (defaults to 1000),
        both vector and matrix norm type (defaults to "fro" (Frobenius- and 2-norm)) and
        start point for monitoring (defaults to 0).
        """
        super().__init__()
    ...
    def on_epoch_end(self):
        ...
        if (
            abs(
                (max(self.norm_list) - min(self.norm_list))
                * len(self.norm_list)
                / sum(self.norm_list)
            )
            < self.min_delta
    ...
    def on_train_end(self):
        ...
        if self.stopped_iteration > 0:
            print(
                f"Early stopping at iteration {self.stopped_iteration} as relative change of {self.norm_choice} norm was below {self.min_delta:.1E} for at least {self.patience} iterations."
            )

    def get_norm_value(self):
        """Get the monitored norms of the bias vector and weights matrix."""
...
class ResetLagrangeParameters(Callback):
    ...
    This is necessary, as the lagrange parameters are only initialized as zero with the geometry creation.
    """

    def __init__(self):
        """Initialize ResetLagrangeParameters."""
        super().__init__()

    def on_train_begin(self):
        """Reset Lagrange parameters at the beginning of the training."""
        if not self.model.data.geom.lagrange_method:
            raise ValueError(
                "You are requesting a reset of lagrange parameters even though you are not using a lagrange method."
            )
        self.model.data.geom.lagrange_parameter = np.zeros_like(
            self.model.data.geom.lagrange_parameter
        )
...
class UpdateLagrangeParameters(Callback):
    ...
    def __init__(self, epsilon):
        """Initialize UpdateLagrangeParameters.
        Set the penalty parameter epsilon.
        """
        super().__init__()
        self.epsilon = float(epsilon)

    def on_train_end(self):
        """Update the Lagrange parameters according to a Uzawa rule."""
        ...
        mapped_normal_boundary_t = torch.as_tensor(
            self.model.data.geom.mapped_normal_boundary, device=device, dtype=dtype
        )
        ...
        gap_n = calculate_gap_in_normal_direction_deep_energy(
            inputs[beg_boundary:], outputs[beg_boundary:], X, current_normals, cond
        )
        # compute the new lagrange parameters
        lagrange_parameter = torch.as_tensor(
            self.model.data.geom.lagrange_parameter, device=device, dtype=dtype
        )
        lagrange_parameter[cond] = torch.relu(
            lagrange_parameter[cond] - self.epsilon * gap_n
        )
        self.model.data.geom.lagrange_parameter = (
            lagrange_parameter.detach().cpu().numpy()
        )
        print(
            f"Updating lagrange parameters. Current length of lagrange parameter vector: {np.linalg.norm(lagrange_parameter.detach().cpu().numpy())}"
        )
