"""
Module with utility functions for applying numerical integration using mainly Gaussian quadrature.
"""
...
class GaussQuadratureRule:
    def __init__(
        self, rule_name, dimension, ngp, element_type="tensor", **additional_params
    ):
        """
    ...
    def generate(self):
        """
        Generate quadrature points and weights based on the specified rule and element type.
        """
    ...
    def generate_simplex(self):
        ...
        if self.dimension == 2:
            ...
            raise NotImplementedError(
                "Only 2D triangle and 3D tetrahedron supported in simplex mode"
            )
    ...
    def triangle_rule(self):
        ...
        if self.ngp == 1:
            ...
            coords = np.array([[1 / 6, 1 / 6], [2 / 3, 1 / 6], [1 / 6, 2 / 3]])
            ...
            raise NotImplementedError(
                "Triangle quadrature rule not implemented for ngp = {}".format(self.ngp)
            )
    ...
    def tetrahedron_rule(self):
        ...
        if self.ngp == 1:
            ...
            a = 0.58541020
            b = 0.13819660
            coords = np.array([[b, b, b], [a, b, b], [b, a, b], [b, b, a]])
            weights = np.array(
                [[1 / 24], [1 / 24], [1 / 24], [1 / 24]]
            )  # All weights equal, total volume = 1/6
            ...
            raise NotImplementedError(
                f"{self.ngp}-point tetrahedron quadrature not implemented"
            )
    ...
    def gauss_legendre(self):
        ...
        if self.dimension == 1:
            ...
            coord_quadrature_x, coord_quadrature_y = np.meshgrid(
                coord_quadrature_1d, coord_quadrature_1d
            )
            coord_quadrature_2d = np.array(
                (coord_quadrature_x.ravel(), coord_quadrature_y.ravel())
            ).T

            weight_quadrature_x, weight_quadrature_y = np.meshgrid(
                weight_quadrature_1d, weight_quadrature_1d
            )
            weight_quadrature_2d = np.array(
                (weight_quadrature_x.ravel(), weight_quadrature_y.ravel())
            ).T
            ...
            coord_quadrature_x, coord_quadrature_y, coord_quadrature_z = np.meshgrid(
                coord_quadrature_1d, coord_quadrature_1d, coord_quadrature_1d
            )
            coord_quadrature_3d = np.array(
                (
                    coord_quadrature_x.ravel(),
                    coord_quadrature_y.ravel(),
                    coord_quadrature_z.ravel(),
                )
            ).T

            weight_quadrature_x, weight_quadrature_y, weight_quadrature_z = np.meshgrid(
                weight_quadrature_1d, weight_quadrature_1d, weight_quadrature_1d
            )
            weight_quadrature_3d = np.array(
                (
                    weight_quadrature_x.ravel(),
                    weight_quadrature_y.ravel(),
                    weight_quadrature_z.ravel(),
                )
            ).T
    ...
    def gauss_lobatto(self):
        ...
        if self.dimension == 1:
            ...
            coord_quadrature_x, coord_quadrature_y, coord_quadrature_z = np.meshgrid(
                coord_quadrature, coord_quadrature, coord_quadrature
            )
            coord_quadrature_3d = np.array(
                (
                    coord_quadrature_x.ravel(),
            ...
            weight_quadrature_x, weight_quadrature_y, weight_quadrature_z = np.meshgrid(
                weight_quadrature, weight_quadrature, weight_quadrature
            )
            weight_quadrature_3d = np.array(
                (
                    weight_quadrature_x.ravel(),
...
def get_test_function_properties(n_test, coord_quadrature, approach="2"):
    ...
    dict_legendre_derivative = {
        "1": modified_legendre_derivative,
    ...
    for i in range(1, n_test + 1):
        ...
        test_function_derivative.append(
            dict_legendre_derivative[approach](i, coord_quadrature)
        )

    return np.array(test_function).astype(config.real(np)), np.array(
        test_function_derivative
...
def modified_legendre(n, x):
    ...
    return GaussQuadratureRule.jacobi_polynomial(
        n + 1, 0, 0, x
    ) - GaussQuadratureRule.jacobi_polynomial(n - 1, 0, 0, x)


def modified_legendre_2(n, x):
    """
    Generate quadrature points and weights according to the modified gauss legendre rule based on http://hyperphysics.phy-astr.gsu.edu/hbase/Math/legend.html.
    """

    def legendre_p(n, x):
        """define legendre polynomial of order n."""
        sum_p = 0
        M = n // 2
...
def modified_legendre_derivative_2(n, x):
    """
    Generate derivatives of the functional according to the modified gauss legendre rule based on http://hyperphysics.phy-astr.gsu.edu/hbase/Math/legend.html.
    """
...
def modified_legendre_derivative(n, x):
    """
    Generate derivatives of the functional according to the modified gauss legendre rule based on VPINNs paper.
    """
    if n == 1:
        d1 = ((1 + 2) / 2) * GaussQuadratureRule.jacobi_polynomial(1, 1, 1, x)
        d2 = ((1 + 2) * (1 + 3) / (2 * 2)) * GaussQuadratureRule.jacobi_polynomial(
            0, 2, 2, x
        )
    elif n == 2:
        ...
        d2 = ((2 + 2) * (2 + 3) / (2 * 2)) * GaussQuadratureRule.jacobi_polynomial(
            1, 2, 2, x
        )
    else:
        ...
        d2 = ((n + 2) * (n + 3) / (2 * 2)) * GaussQuadratureRule.jacobi_polynomial(
            n - 1, 2, 2, x
        ) - ((n) * (n + 1) / (2 * 2)) * GaussQuadratureRule.jacobi_polynomial(
            n - 3, 2, 2, x
        )
