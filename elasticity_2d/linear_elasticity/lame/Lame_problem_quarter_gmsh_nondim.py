...
def compareModelPredictionAndAnalyticalSolution(model):
    ...
    disps = (
        model.predict(r_x) * characteristic_disp / characteristic_length
    )  # dimensionalize the displacement using characteristic_disp, the last term, characteristic_length, should not be there?
...
displacement = (
    model.predict(X) * characteristic_disp / characteristic_length
)  # dimensionalize the displacement using characteristic_disp, the last term, characteristic_length, should not be there?
