...
log_dir = "logs/pinn_visualization"
os.makedirs(log_dir, exist_ok=True)

# Custom TensorBoard callback initialization
class CustomTensorBoard(tf.keras.callbacks.TensorBoard):
    def __init__(self, log_dir, **kwargs):
        super().__init__(log_dir, **kwargs)
        self.writer = tf.summary.create_file_writer(log_dir)  # Manually create a writer

    def on_epoch_end(self, epoch, logs=None):
        """ Override the on_epoch_end to ensure TensorBoard callback works properly"""
        with self.writer.as_default():
            for name, value in logs.items():
                # Convert np.ndarray to scalar values if needed
                if isinstance(value, np.ndarray):
                    value = value.item()  # Convert to a scalar value
                tf.summary.scalar(name, value, step=epoch)
            self.writer.flush()  # Manually flush the writer to save the summaries

# Initialize model (ensure `data` and `net` are defined somewhere)
layer_size = [2] + [50] * 3 + [6]  # Input layer (2), hidden layers (3 x 50 units), output (6 variables)
activation = "swish"  # Swish activation function
initializer = "Glorot uniform"  # Weight initializer
net = dde.maps.FNN(layer_size, activation, initializer)

# Initialize the model (make sure data is set)
...
tensorboard_callback = CustomTensorBoard(log_dir=log_dir, histogram_freq=0, profile_batch=0)
callbacks = [tensorboard_callback]

# Enter your 12 unique names for the loss functions here, in the order you want them to appear:
loss_names = [
    "PDE_loss_1",  
    "PDE_loss_2",  
    "CC_loss_Pxx",  
...
# Enter your custom names for the inputs and outputs
input_names = ["Coord_x", "Coord_y"]  # Replace these with your custom names
output_names = ["u_x", "u_y", "Pxx", "Pyy", "Pxy", "Pyx"]  # Replace these with your custom names

# Manually loop over epochs
num_epochs = 10

for epoch in range(num_epochs):
    # Call on_epoch_begin for each epoch
    for callback in callbacks:
        callback.on_epoch_begin(epoch)

    # Perform a single training step (DeepXDE's internal mechanism)
    losshistory, train_state = model.train(epochs=1, display_every=200)  # Training for one step
    
    # Dynamically create a dictionary of losses using enumeration
    logs = {}

    # Assuming `losshistory.loss_train` has 12 components, and we are assigning distinct names
    for i, loss_array in enumerate(losshistory.loss_train):
        for j, loss in enumerate(loss_array):
            # Assign each loss function a distinct name from `loss_names` in order
            if i * len(loss_array) + j < len(loss_names):
                logs[loss_names[i * len(loss_array) + j]] = loss

    # Get the inputs and outputs during training to visualize them
    input_values = model.data.train_x_all  # Get input values from the training data
    output_values = model.predict(input_values)  # Get predictions from the model

    # Add summaries for input and output to logs
    with tensorboard_callback.writer.as_default():
        # Input Data Summaries
        for idx, name in enumerate(input_names):
            tf.summary.histogram(name, input_values[:, idx], step=epoch)  # Assuming each input has its own column

        # Output Data Summaries
        for idx, name in enumerate(output_names):
            tf.summary.histogram(name, output_values[:, idx], step=epoch)  # Assuming each output has its own column

    # After training the epoch, call on_epoch_end
    for callback in callbacks:
        callback.on_epoch_end(epoch, logs)  # Pass the logs dictionary containing all the losses

# model.compile("L-BFGS")
# model.train(model_save_path=None, display_every=200)

# model.compile("L-BFGS")
# model.train(model_save_path=None, display_every=200, callbacks=[tensorboard_callback, ActivationLogger(model)])

# Run TensorBoard with: tensorboard --logdir=logs/pinn_visualization
