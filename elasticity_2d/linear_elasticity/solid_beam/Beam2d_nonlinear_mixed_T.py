...
from utils.elasticity.elasticity_utils import problem_parameters, first_piola_stress_tensor, momentum_mixed_T, problem_parameters, zero_neumann_x_mixed_T_formulation, zero_neumann_y_mixed_T_formulation, cauchy_stress
...
block_2d = Block_2D(coord_left_corner=[-width/2,-height/2], coord_right_corner=[width/2,height/2], mesh_size=0.05, gmsh_options=gmsh_options) #0.095
...
bc1 = dde.OperatorBC(geom, zero_neumann_x_mixed_T_formulation, top_bottom_right)
bc2 = dde.OperatorBC(geom, zero_neumann_y_mixed_T_formulation, top_bottom)
...
# two inputs x and y, output is ux, uy, Txx, Tyy, Txy
layer_size = [2] + [50] * 3 + [5]
activation = "swish"
...
# displacement = model.predict(X[:,0:2])    Only works with displacement based approach
...
#T_xx, T_yy, T_xy, T_yx = model.predict(X[:,0:2], operator=cauchy_stress)       Only works with displacement based approach
...
data.save(f"Beam2D_mixed_T_{model_complexity}_u_{applied_displacement:.1f}_{activation}_{model_type}.vtu")
