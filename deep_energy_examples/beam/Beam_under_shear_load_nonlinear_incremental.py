...
from utils.postprocess.custom_callbacks import EpochTracker, SaveModelVTU
...
model_saver_incremental = SaveModelVTU(op=cauchy_stress_2D, period=1000, stabilization_epoch=stabilization_model_epoch, filename=file_path)
...
model_saver = SaveModelVTU(op=cauchy_stress_2D, period=500, stabilization_epoch=(epochs+stabilization_model_epoch), filename=file_path)

apply_load = True

model.compile("adam", lr=0.001)
losshistory, train_state = model.train(epochs=10000, callbacks=[model_saver], display_every=100)
