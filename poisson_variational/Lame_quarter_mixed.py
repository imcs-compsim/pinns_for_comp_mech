...
def weak_form(inputs, outputs, beg, n_e, n_gp, g_jacobian, g_weights, g_test_function, g_test_function_derivative):
    ...
    if residual_form == "1":
        ...
        # sigma_xx_xy2 = dde.grad.jacobian(sigma_xx_x, inputs, j=1)
        # sigma_yy_yx = dde.grad.jacobian(sigma_yy_y, inputs, j=0)
        # sigma_xy_xy = dde.grad.jacobian(sigma_xy_x, inputs, j=1)
        
        residual = vx*vy*(sigma_xx_xx[beg:]+sigma_xy_xy[beg:] + 
                           sigma_xx_xy[beg:]+sigma_xy_yy[beg:] + 
                           sigma_xy_xx[beg:]+sigma_yy_xy[beg:] + 
                           sigma_xy_xy[beg:]+sigma_yy_yy[beg:]) + residual_1
    
    weighted_residual = g_weights[:,0:1]*g_weights[:,1:2]*(residual)*g_jacobian
